...
export const NameBoxDropdown = memo(function NameBoxDropdown({
  ...
  const handleNameBoxClick = useCallback(() => {
    if (!isEditing) {
      ...
      inputValueRef.current = cellAddress;
  ...
  const navigateToAddress = useCallback(
    async (address: string, options: { allowNameCreation?: boolean } = {}) => {
      ...
      if (options.allowNameCreation !== false && nameValidation.valid) {
        const currentRange = ranges[0];
  ...
  const handleInputKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        const typed = e.currentTarget.value ?? inputValueRef.current;
  ...
  const handleInputBlur = useCallback((e: React.FocusEvent<HTMLInputElement>) => {
    const typed = e.currentTarget.value || inputValueRef.current;
    if (parseCellRange(typed.trim())) {
      void navigateToAddress(typed, { allowNameCreation: false });
    }
    isEditingRef.current = false;
    setIsEditing(false);
    setIsOpen(false);
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <Popover open={isOpen && !isEditing} onOpenChange={handlePopoverOpenChange}>
        {isEditing ? (
