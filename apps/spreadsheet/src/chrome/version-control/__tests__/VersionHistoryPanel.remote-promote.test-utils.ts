...
type CreateRemotePromotionWorkbookOptions = {
  ...
  readonly promotePendingRemote?: VersionHistoryWorkbook['version']['refs']['promotePendingRemote'];
};
...
): Awaited<ReturnType<VersionHistoryWorkbook['version']['refs']['promotePendingRemote']>> {
  return {
    ok: true,
    value: {
      status: 'success',
