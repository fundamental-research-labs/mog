export const DIALOG_ACTION_APPLY_DELAY_MS = 50;
...
function setPendingDialogAction<T>(pending: Promise<T>): void {
  const global = getDialogActionGlobal();
  const tracked = pending.then(
    () => {
      if (global.__MOG_PENDING_DIALOG_ACTION__ === tracked) {
  ...
  global.__MOG_PENDING_DIALOG_ACTION__ = tracked;
}

export function trackDialogAction<T>(action: () => T | Promise<T>): Promise<T> {
  const pending = Promise.resolve().then(action);
  setPendingDialogAction(pending);
  return pending;
}
