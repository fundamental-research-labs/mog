//! Unit tests for show_values_as transform functions.
//!
//! These tests exercise each `ShowValuesAs` variant (PercentOfGrandTotal,
//! PercentOfColumnTotal, PercentOfRowTotal, Difference, PercentDifference,
//! RunningTotal, Index, RankAscending, RankDescending, PercentOfParentRowTotal,
//! PercentOfParentColumnTotal) **in isolation** -- i.e., the transform
//! functions are called directly on pre-built `PivotRow` / hierarchy data
//! without going through the full `compute` pipeline.
//!
//! For end-to-end tests that verify show_values_as through the full
//! compute pipeline, see `engine/show_values_as_integration_tests.rs`.

use super::*;
use crate::hierarchy::{GroupHierarchy, build_group_hierarchy};
use crate::types::{FieldId, PivotHeader, RelativePosition, ShowValuesAsBaseItem};
...
fn make_grand_totals(
    row: Option<Vec<CellValue>>,
    grand: Option<Vec<CellValue>>,
) -> PivotGrandTotals {
    PivotGrandTotals {
        row,
        column: None,
...
fn make_two_level_test_data() -> (Vec<PivotRow>, GroupHierarchy, PivotGrandTotals) {
    ...
    let grand_totals = make_grand_totals(
        Some(vec![CellValue::number(700.0)]),
        Some(vec![CellValue::number(700.0)]),
    );
...
fn make_flat_test_data() -> (Vec<PivotRow>, GroupHierarchy, PivotGrandTotals) {
    ...
    let hierarchy = build_group_hierarchy(&rows, &field_names);
    let grand_totals = make_grand_totals(
        Some(vec![CellValue::number(500.0)]),
        Some(vec![CellValue::number(500.0)]),
    );
...
fn test_percent_difference_zero_base() {
    ...
    let hierarchy = build_group_hierarchy(&rows, &field_names);
    let grand_totals = make_grand_totals(
        Some(vec![CellValue::number(100.0)]),
        Some(vec![CellValue::number(100.0)]),
    );
...
fn test_rank_ascending_with_ties() {
    ...
    let hierarchy = build_group_hierarchy(&rows, &field_names);
    let grand_totals = make_grand_totals(
        Some(vec![CellValue::number(400.0)]),
        Some(vec![CellValue::number(400.0)]),
    );
...
fn test_index_flat() {
    ...
    let hierarchy = build_group_hierarchy(&rows, &field_names);
    let grand_totals = make_grand_totals(
        Some(vec![CellValue::number(200.0)]),
        Some(vec![CellValue::number(200.0)]),
    );
...
fn make_three_level_test_data() -> (Vec<PivotRow>, GroupHierarchy, PivotGrandTotals) {
    ...
    let hierarchy = build_group_hierarchy(&rows, &field_names);
    let grand_totals = make_grand_totals(
        Some(vec![CellValue::number(1080.0)]),
        Some(vec![CellValue::number(1080.0)]),
    );
...
fn test_multi_column_running_total() {
    ...
    let hierarchy = build_group_hierarchy(&rows, &field_names);
    let grand_totals = make_grand_totals(
        Some(vec![CellValue::number(660.0)]),
        Some(vec![CellValue::number(660.0)]),
    );
...
fn test_multi_value_field_transforms() {
    ...
    let hierarchy = build_group_hierarchy(&rows, &field_names);
    let grand_totals = make_grand_totals(
        Some(vec![CellValue::number(800.0), CellValue::number(90.0)]),
        Some(vec![CellValue::number(800.0), CellValue::number(90.0)]),
    );
...
fn test_single_data_row_all_transforms() {
    ...
    let hierarchy = build_group_hierarchy(&rows, &field_names);
    let grand_totals = make_grand_totals(
        Some(vec![CellValue::number(42.0)]),
        Some(vec![CellValue::number(42.0)]),
    );
...
fn test_running_total_with_nulls() {
    ...
    let hierarchy = build_group_hierarchy(&rows, &field_names);
    let grand_totals = make_grand_totals(
        Some(vec![CellValue::number(300.0)]),
        Some(vec![CellValue::number(300.0)]),
    );
...
// =======================================================================
// Bug reproduction: ShowValuesAs transforms skip subtotal / grand total rows
// See SHOW-VALUES-AS-SUBTOTALS.md
//
// The transforms iterate `ctx.data_row_indices` which excludes subtotals
// and grand totals, leaving those rows with their raw aggregated values.
// These tests are expected to FAIL until the bug is fixed.
// =======================================================================

#[test]
fn test_percent_of_parent_row_total_transforms_subtotals() {
    // Reproduces bug: subtotal/grand-total rows keep raw values instead of
    // being transformed by PercentOfParentRowTotal.
    let (mut rows, hierarchy, grand_totals) = make_two_level_test_data();
    ...
    // East subtotal (row 2): parent is grand total → 300/700 ≈ 0.4286
    assert_approx(num(&rows[2].values[0]), 300.0 / 700.0, 0.001);
    // West subtotal (row 5): parent is grand total → 400/700 ≈ 0.5714
    assert_approx(num(&rows[5].values[0]), 400.0 / 700.0, 0.001);
    // Grand total (row 6): top-level parent → 1.0
...
fn test_percent_of_row_total_transforms_subtotals() {
    // Reproduces bug: subtotal/grand-total rows keep raw values instead of
    // being transformed by PercentOfRowTotal.
    // Single-column layout so row_total == the cell value → all should be 1.0.
    let (mut rows, hierarchy, grand_totals) = make_two_level_test_data();
...
fn test_percent_of_parent_column_total_transforms_subtotals() {
    // Reproduces bug: subtotal/grand-total rows keep raw values instead of
    // being transformed by PercentOfParentColumnTotal.
    let (mut rows, hierarchy, grand_totals) = make_two_level_test_data();
    ...
    // East subtotal (row 2): parent column total is grand total → 300/700
    assert_approx(num(&rows[2].values[0]), 300.0 / 700.0, 0.001);
    // West subtotal (row 5): → 400/700
    assert_approx(num(&rows[5].values[0]), 400.0 / 700.0, 0.001);
    // Grand total (row 6): should be 1.0
    assert_approx(num(&rows[6].values[0]), 1.0, 0.001);
}

#[test]
fn test_running_total_transforms_subtotals() {
    // Reproduces bug: collect_groups_at_depth only uses data_row_indices,
    // so subtotals/grand totals keep their raw aggregated values.
    let (mut rows, hierarchy, grand_totals) = make_two_level_test_data();
    ...
    // Subtotals should receive running-total values among peer subtotals.
    // Peer subtotals at depth 0: [East=300, West=400]
    // Running total: [300, 700]
    // East subtotal (row 2): first in group, running total = 300 (same as raw)
    assert_approx(num(&rows[2].values[0]), 300.0, 1e-10);
    // West subtotal (row 5): running total = 300 + 400 = 700 (NOT raw 400)
...
fn test_rank_transforms_subtotals() {
    // Reproduces bug: collect_groups_at_depth only uses data_row_indices,
    // so subtotals receive no rank value.
    let (mut rows, hierarchy, grand_totals) = make_two_level_test_data();
    ...
    // Subtotals should receive rank values among peer subtotals, not raw values.
    // East subtotal (row 2): raw was 300.0 — should now be a rank
    assert!(
        num(&rows[2].values[0]) != 300.0,
        "East subtotal should be ranked, not raw 300.0"
    );
    // West subtotal (row 5): raw was 400.0 — should now be a rank
    assert!(
        num(&rows[5].values[0]) != 400.0,
        "West subtotal should be ranked, not raw 400.0"
    );
    // Grand total (row 6): raw was 700.0 — should now be a rank
    assert!(
...
fn test_index_transforms_subtotals() {
    // Reproduces bug: apply_index iterates data_row_indices only,
    // so subtotals/grand totals keep raw values.
    let (mut rows, hierarchy, grand_totals) = make_two_level_test_data();
    ...
    // Subtotals should get index values, not raw values.
    // East subtotal (row 2): raw was 300.0
    assert!(
        num(&rows[2].values[0]) != 300.0,
        "East subtotal should have index value, not raw 300.0"
    );
    // West subtotal (row 5): raw was 400.0
    assert!(
        num(&rows[5].values[0]) != 400.0,
        "West subtotal should have index value, not raw 400.0"
    );
    // Grand total (row 6): Index of grand total = 1.0
    assert_approx(num(&rows[6].values[0]), 1.0, 0.001);
}
