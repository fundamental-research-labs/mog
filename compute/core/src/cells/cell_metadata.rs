...
/// Storage implements this interface; the cell store/evaluator never own a
/// mutable storage interface. Queries use the current cell-store identities and
...
pub(crate) trait CellMetadataProvider: std::fmt::Debug + Send + Sync {
    ...
    fn row_hidden(&self, _store: &CellStore, _sheet: &SheetId, _row: u32) -> Option<bool> {
        None
    }
    fn is_row_filtered(&self, _store: &CellStore, _sheet: &SheetId, _row: u32) -> bool {
        false
    }
