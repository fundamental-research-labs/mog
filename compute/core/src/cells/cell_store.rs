//! The top-level CellStore struct definition.
...
pub struct CellStore {
    ...
    /// Ownership of explicit row identities only; compact rows use run ownership.
    pub(super) row_to_sheet: FxHashMap<RowId, SheetId>,
    /// Ownership of explicit column identities only; compact columns use run ownership.
...
impl CellStore {
    ...
    /// `cell_store.data_table_regions` directly.
    ///
    ...
    ///
    /// Backs [`crate::cells::read::StorePositionLookup::row_index`]. Returns
    /// `None` if the row was never populated into the cell store or has since been
        ...
        for sheet_id in sheet_ids {
            ...
            if let Some(sheet) = self.sheets.get(&sheet_id) {
                for rv in sheet.range_views.values() {
                        ...
                        for row_id in rv.row_offset_by_id.keys() {
                            for col_id in rv.col_offset_by_id.keys() {
                                        ...
                                        sheet.col_axis.position_of(sheet.id, col_id)
                                {
                                    ...
                                    if !sheet.authored_cell_id_at(pos).is_some() {
                    ...
                    if let (Some((start_row, end_row)), Some((start_col, end_col))) = (
                        rv.row_offset_by_id
                            .position_bounds(sheet.id, &sheet.row_axis),
                        rv.col_offset_by_id
                            .position_bounds(sheet.id, &sheet.col_axis),
                    ) {
...
// ---------------------------------------------------------------------------
// DataSource implementation — bridges CellStore to the eval cache layer
// ---------------------------------------------------------------------------
...
impl DataSource for CellStore {
    ...
    fn cell_id_at(&self, sheet: &SheetId, row: u32, col: u32) -> Option<CellId> {
        CellStore::resolve_cell_id(self, sheet, SheetPos::new(row, col))
    }

    fn sheet_for_cell(&self, cell_id: &CellId) -> Option<SheetId> {
        CellStore::sheet_for_cell(self, cell_id)
    }

    fn sheet_by_name(&self, name: &str) -> Option<SheetId> {
        CellStore::sheet_by_name(self, name)
    }

    fn get_cell_value_at(&self, sheet: &SheetId, row: u32, col: u32) -> Option<&CellValue> {
        CellStore::get_cell_value_at(self, sheet, SheetPos::new(row, col))
    }
...
// ---------------------------------------------------------------------------
// PositionResolver implementation — bridges CellStore to compute-graph
...
impl PositionResolver for CellStore {
    fn resolve(&self, cell_id: &CellId) -> Option<CellPosition> {
        ...
        let pos = sheet_store.position_of(cell_id)?;
        Some(CellPosition {
            sheet,
            row: pos.row(),
            col: pos.col(),
