...
use super::test_helpers::fresh_store_with_sheet;
...
fn materialize_pivot_writes_transformed_grand_total_branches() {
    let (mut cell_store, sheet_id) = fresh_store_with_sheet(20, 20);
...
fn materialize_outline_multi_row_fields_can_suppress_repeated_outer_labels() {
    ...
    cell_store.materialize_pivot_with_row_label_options(
        &sheet_id,
        ANCHOR_ROW,
        ANCHOR_COL,
        &result,
    ...
    assert!(
        matches!(
            cell_store.get_cell_value_at(&sheet_id, SheetPos::new(ANCHOR_ROW + 2, ANCHOR_COL)),
