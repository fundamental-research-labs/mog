...
mod tests {
    ...
    fn find_pivot_table_at_uses_imported_uuid_sheet_identity() {
        ...
        cell_store.add_sheet_store(
            sheet_id,
            "Imported".to_string(),
            SheetStore::new(sheet_id, "Imported".to_string(), 20, 20),
        );
        cell_store.upsert_pivot_table_def(pivot_def("ImportedPivot", &sheet_id, 4, 3));

        let found = cell_store
    ...
    fn find_pivot_table_at_distinguishes_same_name_by_output_sheet_uuid() {
        ...
        cell_store.add_sheet_store(
            sheet_a,
            "RuntimeA".to_string(),
            SheetStore::new(sheet_a, "RuntimeA".to_string(), 20, 20),
        );
        cell_store.add_sheet_store(
            sheet_b,
            "RuntimeB".to_string(),
            SheetStore::new(sheet_b, "RuntimeB".to_string(), 20, 20),
        );
        cell_store.upsert_pivot_table_def(pivot_def("Pivot1", &sheet_a, 0, 0));
        cell_store.upsert_pivot_table_def(pivot_def("Pivot1", &sheet_b, 8, 8));
        ...
        assert!(cell_store.find_pivot_table_at(&sheet_a, 9, 9).is_none());
    }
    ...
    fn pivot_defs_with_same_display_name_on_same_sheet_keep_stable_identities() {
        ...
        cell_store.add_sheet_store(
            sheet_id,
            "Runtime".to_string(),
            SheetStore::new(sheet_id, "Runtime".to_string(), 20, 20),
        );
        ...
        cell_store.upsert_pivot_table_def(first);
        cell_store.upsert_pivot_table_def(second);

        let sheet_uuid = sheet_id.to_uuid_string();
        assert_eq!(cell_store.all_pivot_tables().len(), 2);
        assert_eq!(
            cell_store
