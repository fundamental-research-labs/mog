//! Cell Store — identity-indexed, in-process cell store.
//!
...
//! # Design
//!
//! The cell store provides two lookup paths:
//! - **Identity path** (hot): `CellId -> CellEntry` via `FxHashMap`.
//! - **Positional path** (warm): `(row, col) -> (RowId, ColId) -> CellId`, then identity lookup.
//!
...
//! # Module layout
//!
//! - [`types`] — `CellEntry`, `SheetStore`, `CellEdit` type definitions.
//! - [`cell_store`] — `CellStore` struct definition and constructor.
...
pub use read::StorePositionLookup;
pub use sheet_key::{clear_caches, sheet_name_cache_entry_count};
pub use types::{CellEdit, CellEntry, MergeRegion, SheetStore};
