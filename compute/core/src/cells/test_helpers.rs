//! Shared cell_store test fixtures.
...
/// Construct a fresh `CellStore` containing a single sheet sized
...
pub(crate) fn fresh_store_with_sheet(rows: u32, cols: u32) -> (CellStore, SheetId) {
    let sheet_id = SheetId::from_raw(1);
    let mut cell_store = CellStore::new();
    let sheet_store = SheetStore::new(sheet_id, "Sheet1".to_string(), rows, cols);
...
/// Build a cell store with one sheet and a few cells for structural tests.
pub(crate) fn store_with_grid() -> (CellStore, SheetId) {
    let sheet_id = make_sheet_id(1);
    let mut cell_store = CellStore::new();
    let sheet_store = SheetStore::new(sheet_id, "Grid".to_string(), 10, 5);
    cell_store.add_sheet_store(sheet_id, "Grid".to_string(), sheet_store);
