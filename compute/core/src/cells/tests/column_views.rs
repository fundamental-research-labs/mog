...
use crate::cells::test_helpers::{make_cell_id, store_with_grid};
...
fn test_set_value_mut_invalidates_dense_cache() {
    ...
    cell_store.set_value_mut(&cell_id, CellValue::Number(FiniteF64::must(999.0)));
    assert!(
        cell_store.dense_cache().get(&sheet_id, 0).is_none(),
...
fn test_insert_cell_invalidates_dense_cache() {
    ...
    cell_store.insert_cell(&sheet_id, make_cell_id(500), SheetPos::new(5, 0), entry);
    assert!(
        cell_store.dense_cache().get(&sheet_id, 0).is_none(),
...
#[test]
fn test_remove_cell_clears_column_values_and_dense_cache() {
    let (mut cell_store, sheet_id) = store_with_grid();

    // First populate column_values by applying an edit (which updates column_values)
    let cell_id = make_cell_id(100); // row=0, col=0
    ...
    if let Some(col_vec) = sheet.get_column_view(0) {
        if !col_vec.is_empty() {
            assert_eq!(
                col_vec[0],
                CellValue::Null,
                "column_values should be cleared to Null after remove_cell"
            );
        }
    }
}
#[test]
fn test_column_values_grows_on_out_of_bounds_insert() {
    let (mut cell_store, sheet_id) = store_with_grid();

    // The grid has cells at rows 0-2, cols 0-2. column_values may have vectors of length ~3.
    // Insert a cell at row 8 (beyond column_values vector length) — should grow, not silently drop.
    let entry = CellEntry {
    ...
    if let Some(col_vec) = sheet.get_column_view(0) {
        ...
        assert_eq!(
            ...
            "column_values should contain the inserted value at row 8"
        );
    }
}
#[test]
fn test_column_values_grows_on_set_value_mut() {
    let (mut cell_store, sheet_id) = store_with_grid();

    // Insert a cell at a high row that won't be in column_values initially
    let cell_id = make_cell_id(900);
    let entry = CellEntry {
        value: CellValue::Number(FiniteF64::must(1.0)),
    };
    cell_store.insert_cell(&sheet_id, cell_id, SheetPos::new(9, 0), entry);

    // Now set_value_mut at row 9 — the column_values vector should grow to accommodate
...
#[test]
fn test_column_values_rebuilt_after_insert_rows() {
    let (mut cell_store, sheet_id) = store_with_grid();

    // Populate column_values via apply_edit
    cell_store.apply_edit(
        &sheet_id,
        make_cell_id(100),
        SheetPos::new(0, 0),
    ...
    if let Some(col_vec) = sheet.get_column_view(0) {
        ...
        if col_vec.len() > 3 {
            assert_eq!(
                col_vec[3],
                CellValue::Number(FiniteF64::must(20.0)),
                "shifted row value should be at new position in column_values"
            );
        }
    }
}
#[test]
fn test_column_values_rebuilt_after_delete_rows() {
    let (mut cell_store, sheet_id) = store_with_grid();
    ...
    if let Some(col_vec) = sheet.get_column_view(0) {
        assert_eq!(
            col_vec[0],
            CellValue::Number(FiniteF64::must(10.0)),
            "after delete, shifted cell should be at new position in column_values"
        );
    }
}
#[test]
fn test_column_values_padded_to_sheet_rows_after_insert() {
    // Regression: rebuild_column_values must pad vectors to sheet.rows
    // (matching snapshot load invariant), not just to last-occupied-row+1.
    let (mut cell_store, sheet_id) = store_with_grid();
    // store_with_grid: 3×3 grid in a 10×5 sheet, rows 0-2 occupied
    ...
    for col in sheet.column_lengths.keys() {
        ...
        assert_eq!(
            col_vec.len(),
            sheet.rows as usize,
            "column_values[{col}] should be padded to sheet.rows ({}) but was {}",
            sheet.rows,
            col_vec.len(),
        );
    }
...
#[test]
fn test_column_values_padded_to_sheet_rows_after_delete() {
    let (mut cell_store, sheet_id) = store_with_grid();
    // Delete row 0
    let deleted = vec![make_cell_id(100), make_cell_id(101), make_cell_id(102)];
    cell_store.apply_structure_change(
