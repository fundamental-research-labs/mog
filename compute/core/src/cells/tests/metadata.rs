...
use crate::cells::test_helpers::{make_cell_id, make_sheet_id};
...
fn test_named_range_crud() {
    ...
    cell_store.set_named_range("MyRange".to_string(), def);

    // Case-insensitive lookup
    assert!(cell_store.get_named_range("myrange").is_some());
    assert!(cell_store.get_named_range("MYRANGE").is_some());
    assert!(cell_store.get_named_range("MyRange").is_some());

    let nr = cell_store.get_named_range("myrange").unwrap();
    assert_eq!(nr.refers_to.refs.len(), 1);

    // Remove
    cell_store.remove_named_range("MYRANGE");
    assert!(cell_store.get_named_range("myrange").is_none());
}
...
fn test_table_case_insensitive_remove() {
    ...
    cell_store.remove_table("SALES");
    assert!(
        cell_store.get_table("Sales").is_none(),
