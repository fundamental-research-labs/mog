...
impl CellStore {
    ...
    pub fn set_formula(&mut self, cell_id: &CellId, formula: Option<IdentityFormula>) -> bool {
        if let Some(sheet_id) = self.cell_to_sheet.get(cell_id)
            ...
            && sheet.cells.contains_key(cell_id)
        {
            if let Some(formula) = formula {
                sheet.formulas.insert(*cell_id, formula);
            } else {
                sheet.formulas.remove(cell_id);
            }
            return true;
        }
        false
    ...
    fn insert_cell_with_formula(
            ...
            .filter(|old| old != sheet)
        {
            let old_pos = if let Some(old_sheet) = self.sheets.get_mut(&old_sheet_id) {
                ...
                old_sheet.formulas.remove(&cell_id);
        ...
        if let Some(s) = self.sheets.get_mut(sheet) {
            ...
            if let Some(formula) = formula {
                s.formulas.insert(cell_id, formula);
            } else {
    ...
    pub fn move_cells(&mut self, moves: &[(CellId, SheetId, SheetPos)]) {
        ...
        for &(cell_id, destination, pos) in moves {
            ...
            let Some(source) = self.sheets.get_mut(&source_id) else {
                continue;
            };
            ...
            let inherited = source
                ...
                .flatten();
            let entry = source.cells.remove(&cell_id);
            let entry = inherited.map(|value| CellEntry { value }).or(entry);
            let formula = source.formulas.remove(&cell_id);
