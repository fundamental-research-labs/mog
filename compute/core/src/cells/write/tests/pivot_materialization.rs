...
fn clear_pivot_region_touches_only_existing_columns() {
    ...
    let before_existing = cell_store.col_version(&sheet_id, 3);
    let before_missing = cell_store.col_version(&sheet_id, 4);
...
fn clear_pivot_region_removes_authored_overrides_that_mask_output() {
    ...
    cell_store.clear_pivot_region(&sheet_id, 2, 3, 1, 1);
...
fn materialized_pivot_output_is_identity_backed_without_overwriting_column_values() {
    ...
    assert!(cell_store.resolve_cell_id(&sheet_id, value_pos).is_some());
    assert_eq!(
        cell_store.get_cell_value_at(&sheet_id, value_pos),
