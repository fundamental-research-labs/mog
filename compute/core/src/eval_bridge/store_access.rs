...
fn classify_name_fallback(raw: &str) -> ResolvedName {
    ...
    //   [`StoreCellRefResolver`](super::StoreCellRefResolver); the
...
impl<'a> MirrorAccess<'a> {
    pub fn new(cell_store: &'a CellStore, current_cell_id: CellId, current_sheet: SheetId) -> Self {
        Self {
            ...
            formula_text_provider: FormulaTextProvider::store_identity_only_for_test_unavailable(),
        }
    }
    ...
    pub fn cell_has_formula(&self, sheet: &SheetId, row: u32, col: u32) -> bool {
        if let Some(cell_id) = self
            ...
            .resolve_cell_id(sheet, SheetPos::new(row, col))
        {
            return self.cell_store.get_formula(&cell_id).is_some();
        }
        false
    }
    ...
    pub fn formula_text_at(&self, sheet: &SheetId, row: u32, col: u32) -> FormulaTextLookup {
        self.formula_text_provider
            .lookup(self.cell_store, sheet, row, col)
    }
    ...
    pub fn cell_has_dynamic_array_formula(&self, sheet: &SheetId, row: u32, col: u32) -> bool {
        if let Some(cell_id) = self
            .cell_store
            .resolve_cell_id(sheet, SheetPos::new(row, col))
            && let Some(formula) = self.cell_store.get_formula(&cell_id)
        {
    ...
    pub fn is_row_hidden(&self, sheet: &SheetId, row: u32) -> bool {
        self.cell_store.is_row_hidden(sheet, row)
    }

    pub fn get_table(&self, name: &str) -> Option<&TableDef> {
        self.cell_store.get_table_def(name)
    }
    ...
    pub fn find_pivot_table_at(
        ...
        self.cell_store.find_pivot_table_at(sheet, row, col)
    }
    ...
    pub fn get_dense_column(&self, sheet: &SheetId, col: u32) -> Option<&DenseColumn> {
        ...
        self.cell_store.dense_cache().get(sheet, col)
    }
    ...
    pub fn get_dense_column_for_range(
        ...
        self.cell_store.dense_cache().get_numeric_for_range(
            *sheet,
            col,
            start_row,
            end_row,
    ...
    pub fn col_version(&self, sheet: &SheetId, col: u32) -> u64 {
        self.cell_store.col_version(sheet, col)
    }

    pub fn get_dense_bool_mask(&self, sheet: &SheetId, col: u32) -> Option<&DenseBoolMask> {
        self.cell_store.dense_cache().get_bool_mask(sheet, col)
    }
    ...
    /// each identity ref to its current position via the cell store. If any ref
    ...
    fn resolve_named_range_def(&self, nr: &NamedRangeDef) -> Option<ResolvedName> {
        ...
        {
            let lookup =
                crate::cells::StorePositionLookup::new(self.cell_store, SheetId::from_raw(0));
        ...
        let positional_result = match &formula.refs[0] {
            ...
            IdentityFormulaRef::RowRange(rr) => {
                let start = self.cell_store.row_index_lookup(&rr.start_row_id);
                let end = self.cell_store.row_index_lookup(&rr.end_row_id);
                match (start, end) {
            ...
            IdentityFormulaRef::ColRange(cr) => {
                let start = self.cell_store.col_index_lookup(&cr.start_col_id);
                let end = self.cell_store.col_index_lookup(&cr.end_col_id);
                match (start, end) {
    ...
    // -----------------------------------------------------------------------
    // Direct value access (from cell_store)
    // -----------------------------------------------------------------------
    ...
    pub fn get_cell_value_by_ref(&self, cell_ref: &CellRef) -> CellValue {
        ...
        match cell_ref {
            CellRef::Resolved(id) => match self.cell_store.get_cell_value(id) {
                Some(v) => v.clone(),
                None => {
                    // If the CellId isn't registered in the cell store at all
                    // (its sheet was deleted), return #REF!. Otherwise it's
                    // just an empty cell -> Null.
                    if self.cell_store.sheet_for_cell(id).is_none() {
    ...
    pub fn get_range_values(
        ...
        if let Some(sheet) = self.cell_store.get_sheet(&s_sheet) {
            ...
            // Sheet not in cell_store — no data to materialize
        ...
        let values = materialize_range(&key, self.cell_store, None);
    ...
    /// Fetch cell values for resolved structured reference ranges from the cell store.
    ///
    ...
    pub fn fetch_structured_ref_values(
        ...
        for range in &resolved.ranges {
            let sheet = self.cell_store.get_sheet(&resolved.sheet);
    ...
    /// Fetch cell value for a resolved defined name from the cell store.
    ///
...
mod tests {
    ...
    fn classify_name_fallback_formula_preserves_bytes() {
        ...
        // because snapshot CellIds don't match cell_store CellIds).
