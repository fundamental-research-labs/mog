//! Formula-parser cell-reference resolution backed by [`CellStore`].
//!
...
/// Resolve formula references against a cell store and an explicit current sheet.
pub(crate) struct StoreCellRefResolver<'a> {
    pub cell_store: &'a CellStore,
    pub current_sheet: SheetId,
}

impl CellRefResolver for StoreCellRefResolver<'_> {
    fn resolve(&self, sheet: &SheetId, row: u32, col: u32) -> CellRef {
            ...
            .resolve_cell_id(sheet, SheetPos::new(row, col))
        {
            ...
            // are intentionally not all registered in the cell store. A resolved
