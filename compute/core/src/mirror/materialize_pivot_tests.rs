...
fn hidden_total_bands_never_allow_corner_aggregate_to_overwrite_visible_cells() {
    for show_row in [false, true] {
        for show_column in [false, true] {
            let (mut mirror, sheet) = fresh_mirror_with_sheet(12, 12);
            let result = PivotTableResult {
                column_headers: vec![],
                rows: vec![
            ...
            mirror.materialize_pivot(&sheet, 2, 1, &result, &["Category".into()]);
            for (row, col, expected) in [(3, 2, 10.0), (3, 3, 20.0), (4, 2, 30.0), (4, 3, 40.0)] {
                assert_eq!(
                    mirror.get_cell_value_at(&sheet, SheetPos::new(row, col)),
                    Some(&num(expected)),
            ...
            if show_row {
                for (col, value) in [(2, 40.0), (3, 60.0)] {
                    assert_eq!(
                        mirror.get_cell_value_at(&sheet, SheetPos::new(5, col)),
                        Some(&num(value))
                    );
                }
            }
            if show_column {
                for (row, value) in [(3, 30.0), (4, 70.0)] {
                    assert_eq!(
                        mirror.get_cell_value_at(&sheet, SheetPos::new(row, 4)),
                        Some(&num(value))
                    );
                }
            }
            if show_row && show_column {
                assert_eq!(
                    mirror.get_cell_value_at(&sheet, SheetPos::new(5, 4)),
                    Some(&num(100.0))
                );
            }
        }
