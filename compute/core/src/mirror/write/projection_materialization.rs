        ...
        if let Some(sheet_mirror) = self.sheets.get_mut(sheet) {
            if let CellValue::Array(array) = array {
                for c in 0..arr_cols {
                    ...
                    let projections = sheet_mirror.projected_columns.entry(col).or_default();
                    projections
                        .retain(|p| p.origin_row != origin_row || p.origin_col != origin_col);
                    projections.push(crate::mirror::types::ProjectionColumn {
                        origin_row,
                        origin_col,
                        array_col: c,
                        array: array.clone(),
    ...
    pub fn clear_materialization(
        ...
        if let Some(sheet_mirror) = self.sheets.get_mut(sheet) {
            for c in 0..cols {
                ...
                if let Some(projections) = sheet_mirror.projected_columns.get_mut(&col) {
                    projections
                        .retain(|p| p.origin_row != origin_row || p.origin_col != origin_col);
                }
                cols_touched.push(col);
