...
impl<'a, 'b> FormulaTextDepCollector<'a, 'b> {
        ...
        match node {
            ASTNode::CellReference(cell) => {
                if let Some((sheet, row, col)) =
                    cell_ref_to_position(&cell.reference, self.sheet_ctx, self.cell_store)
                {
            ...
            ASTNode::Range(range) => {
                let start = cell_ref_to_position(&range.start, self.sheet_ctx, self.cell_store);
                let end = cell_ref_to_position(&range.end, self.sheet_ctx, self.cell_store);
            ...
            ASTNode::Identifier(name) => {
                self.collect_name(name, &[Scope::Sheet(*self.sheet_ctx), Scope::Workbook])
            }
            ASTNode::ExternalNameRef { workbook, name } if workbook.is_current_workbook() => {
                self.collect_name(name, &[Scope::Workbook])
            }
    ...
    fn collect_name(&mut self, name: &str, chain: &[Scope]) -> FormulaTextCollectOutcome {
        if let Some((_var_cell_id, def)) = self.cell_store.variables.resolve_with_id(name, chain) {
            ...
            if let Some(ref_item) = def.refers_to.refs.first() {
                match ref_item {
                    IdentityFormulaRef::Cell(cell_ref) => {
                        if let Some((sheet, row, col)) = self
                            .cell_store
                            .sheet_for_cell(&cell_ref.id)
                            .and_then(|sheet| {
                                self.cell_store
