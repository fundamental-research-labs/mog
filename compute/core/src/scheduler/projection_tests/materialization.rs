...
fn test_resolve_projected_value() {
    ...
    cell_store.materialize_projection(&sheet_id, 5, 3, &arr_value);

    // Test resolve_projected_value at various positions
    // Origin (5,3) -> elem (0,0) = 10.0 (from col_data, written by insert_cell)
    let val = cell_store.resolve_projected_value(&sheet_id, 5, 3);
    assert_eq!(val, Some(CellValue::number(10.0)));

    // (5,4) -> elem (0,1) = 20.0 (from col_data via materialize_projection)
    let val = cell_store.resolve_projected_value(&sheet_id, 5, 4);
    assert_eq!(val, Some(CellValue::number(20.0)));

    // (6,3) -> elem (1,0) = 30.0
    let val = cell_store.resolve_projected_value(&sheet_id, 6, 3);
    assert_eq!(val, Some(CellValue::number(30.0)));

    // (7,4) -> elem (2,1) = 60.0
    let val = cell_store.resolve_projected_value(&sheet_id, 7, 4);
    assert_eq!(val, Some(CellValue::number(60.0)));

    // Outside projection -> None
    let val = cell_store.resolve_projected_value(&sheet_id, 8, 3);
    assert_eq!(val, None);

    let val = cell_store.resolve_projected_value(&sheet_id, 5, 5);
    assert_eq!(val, None);
}
