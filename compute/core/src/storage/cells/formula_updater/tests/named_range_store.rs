use formula_types::{IdentityCellRef, IdentityFormula, IdentityFormulaRef};
use value_types::{CellValue, FiniteF64};

use super::update_store_formulas_on_named_range_rename;
use super::{make_cell_id, make_sheet_id};

#[test]
fn named_range_store_rewrites_template_and_preserves_formula_metadata() {
    let mut storage = crate::storage::WorkbookStorage::new();
    let mut cell_store = crate::cells::CellStore::new();
    let sheet = make_sheet_id(1);
    let cell = make_cell_id(1);
    storage
        .add_sheet(&mut cell_store, sheet, "Sheet1", 10, 5)
        .unwrap();
    cell_store.apply_edit(
        &sheet,
        cell,
        cell_types::SheetPos::new(0, 0),
        CellValue::Number(FiniteF64::must(1.0)),
        Some(IdentityFormula {
            template: "Region+{0}".to_string(),
            refs: vec![IdentityFormulaRef::Cell(IdentityCellRef {
                id: cell,
                row_absolute: false,
                col_absolute: false,
            })],
            is_dynamic_array: true,
            is_volatile: true,
            is_aggregate: true,
        }),
    );

    update_store_formulas_on_named_range_rename(&mut cell_store, "Region", "Sales");

    let formula = cell_store.get_formula(&cell).unwrap();
    assert_eq!(formula.template, "Sales+{0}");
    assert_eq!(
        formula.refs,
        vec![IdentityFormulaRef::Cell(IdentityCellRef {
            id: cell,
            row_absolute: false,
            col_absolute: false,
        })]
    );
    assert!(formula.is_dynamic_array);
    assert!(formula.is_volatile);
    assert!(formula.is_aggregate);
}

#[test]
fn named_range_store_skips_disqualified_contexts() {
    let mut storage = crate::storage::WorkbookStorage::new();
    let mut cell_store = crate::cells::CellStore::new();
    let sheet = make_sheet_id(1);
    let cell = make_cell_id(2);
    storage
        .add_sheet(&mut cell_store, sheet, "Sheet1", 10, 5)
        .unwrap();
    cell_store.apply_edit(
        &sheet,
        cell,
        cell_types::SheetPos::new(0, 0),
        CellValue::Number(FiniteF64::must(1.0)),
        Some(IdentityFormula {
            template: "\"Region\"+Region!{0}+Region({1})+Region[Col]".to_string(),
            refs: vec![],
            is_dynamic_array: false,
            is_volatile: false,
            is_aggregate: false,
        }),
    );

    update_store_formulas_on_named_range_rename(&mut cell_store, "Region", "Sales");

    let formula = cell_store.get_formula(&cell).unwrap();
    assert_eq!(
        formula.template,
        "\"Region\"+Region!{0}+Region({1})+Region[Col]"
    );
}
