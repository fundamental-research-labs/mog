...
pub(super) fn create_slicer(engine: &YrsComputeEngine,
        sheet_id: &SheetId,
        config: StoredSlicer,
...
pub(super) fn delete_slicer(engine: &YrsComputeEngine,
        _sheet_id: &SheetId,
        slicer_id: &str,
    ) -> Result<(Vec<u8>, MutationResult), ComputeError> {
...
pub(super) fn update_slicer_config(engine: &YrsComputeEngine,
        _sheet_id: &SheetId,
        slicer_id: &str,
...
pub(super) fn get_slicer_state(engine: &YrsComputeEngine, _sheet_id: &SheetId, slicer_id: &str) -> Option<StoredSlicer> {
        svc::get_slicer_state(&engine.stores, slicer_id)
    }

pub(super) fn toggle_slicer_item(engine: &YrsComputeEngine,
        _sheet_id: &SheetId,
        slicer_id: &str,
        value: CellValue,
    ) -> Result<(Vec<u8>, MutationResult), ComputeError> {
...
pub(super) fn clear_slicer_selection(engine: &YrsComputeEngine,
        _sheet_id: &SheetId,
        slicer_id: &str,
    ) -> Result<(Vec<u8>, MutationResult), ComputeError> {
...
pub(super) fn map_slicer_invalidation_reason(engine: &YrsComputeEngine,
        reason: &str,
    ) -> Result<slicers::CacheInvalidationEventReason, ComputeError> {
        svc::map_slicer_invalidation_reason(reason)
    }

pub(super) fn map_slicer_disconnection_reason(engine: &YrsComputeEngine,
        reason: &str,
    ) -> Result<slicers::DisconnectionEventReason, ComputeError> {
        svc::map_slicer_disconnection_reason(reason)
    }

pub(super) fn get_slicer_items_from_cache(engine: &YrsComputeEngine, cache: SlicerCache) -> Vec<slicers::SlicerItem> {
        svc::get_slicer_items_from_cache(cache)
    }

pub(super) fn is_slicer_column_connected(engine: &YrsComputeEngine,
...
pub(super) fn find_slicers_for_table(engine: &YrsComputeEngine, slicer_list: Vec<Slicer>, table_id: &str) -> Vec<usize> {
        svc::find_slicers_for_table(&slicer_list, table_id)
    }

pub(super) fn find_disconnected_slicers(engine: &YrsComputeEngine,
