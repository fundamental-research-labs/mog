...
pub(super) fn cf_is_valid_range(_engine: &YrsComputeEngine, range: &CFCellRange) -> bool {
    services::formatting::cf_is_valid_range(range)
}

pub(super) fn get_icon_set_presets(_engine: &YrsComputeEngine) -> Vec<CFIconSetPreset> {
    services::formatting::get_icon_set_presets()
}

pub(super) fn get_cf_preset_by_id(
    _engine: &YrsComputeEngine,
    id: &str,
) -> Option<CFPresetCategory> {
