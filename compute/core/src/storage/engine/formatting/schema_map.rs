use super::*;
    ...
    pub(super) fn remove_schema(engine: &mut YrsComputeEngine, sheet_id: String, column: u32, version: f64) -> bool{
        services::formatting::remove_schema(&mut engine.stores, &sheet_id, column, version)
    }


    pub(super) fn clear_schemas(engine: &mut YrsComputeEngine) -> Result<(Vec<u8>, MutationResult), ComputeError>{
