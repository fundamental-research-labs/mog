use super::*;
    ...
    pub(super) fn get_column_schema(engine: &YrsComputeEngine, sheet_id: &SheetId, col_index: u32) -> Option<ColumnSchema>{
    ...
    pub(super) fn get_all_column_schemas(engine: &YrsComputeEngine, sheet_id: &SheetId) -> Vec<(u32, ColumnSchema)>{
        services::formatting::get_all_column_schemas(&engine.stores, sheet_id)
    }


    pub(super) fn get_range_schema(engine: &YrsComputeEngine, sheet_id: &SheetId, schema_id: &str) -> Option<RangeSchema>{
        services::formatting::get_range_schema(&engine.stores, sheet_id, schema_id)
    }


    pub(super) fn get_range_schemas_for_sheet(engine: &YrsComputeEngine, sheet_id: &SheetId) -> Vec<RangeSchema>{
