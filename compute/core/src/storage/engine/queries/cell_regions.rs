...
pub(in crate::storage::engine) fn get_cell_data(
    ...
    if let Some(mut data) =
        services::queries::get_cell_data(&engine.stores, &engine.cell_store, sheet_id, row, col)
    {
        ...
        data["region"] = region_json(&engine.cell_store, sheet_id, row, col);
        return Some(data);
    }
    let region = region_json(&engine.cell_store, sheet_id, row, col);
    let formula =
        formula_read::formula_text_at(&engine.stores, &engine.cell_store, sheet_id, row, col, None)
            .map(|f| f.strip_prefix('=').unwrap_or(&f).to_string());
    let value = cell_values::get_effective_value(&engine.cell_store, sheet_id, row, col);
    match (value, &region) {
...
pub(in crate::storage::engine) fn get_raw_value(
    ...
    if let Some(formula) = data_table_formula::formula_at(&engine.cell_store, sheet_id, row, col) {
        return formula;
    }
    services::queries::get_raw_value(&engine.cell_store, &engine.stores, sheet_id, row, col)
}
...
pub(in crate::storage::engine) fn get_effective_value(
    ...
    services::queries::get_effective_value(&engine.cell_store, sheet_id, row, col)
}
...
pub(in crate::storage::engine) fn get_cell_count(
    ...
    services::queries::get_cell_count(&engine.cell_store, sheet_id)
}
...
pub(in crate::storage::engine) fn find_last_row(
    ...
    services::queries::find_last_row(&engine.stores, &engine.cell_store, sheet_id, col)
}
...
pub(in crate::storage::engine) fn find_last_column(
    ...
    services::queries::find_last_column(&engine.stores, &engine.cell_store, sheet_id, row)
}
