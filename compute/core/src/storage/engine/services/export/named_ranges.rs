...
pub(super) fn export_workbook_named_ranges(
    ...
    if let Some(Out::Any(Any::String(json))) = stores
        .storage
        .workbook_map()
        .get(&txn, "inertTabDefinedNames")
        && let Ok(inert_names) = serde_json::from_str::<Vec<NamedRange>>(&json)
    {
        named_ranges.extend(inert_names.into_iter().filter_map(|mut name| {
            name.local_sheet_id = Some(*imported_order_to_export_order.get(&name.local_sheet_id?)?);
            Some(name)
        }));
    }
