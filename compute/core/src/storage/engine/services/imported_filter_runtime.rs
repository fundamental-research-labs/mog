...
/// Project an imported date-group criterion for evaluation without mutating storage.
/// Recompile its bounds when workbook date-system
...
pub(in crate::storage::engine) fn project_imported_date_group_filters_for_evaluation(
    ...
    let Some(mut filter) = filters::get_filter(&stores.storage, sheet_id, filter_id) else {
        return None;
    };
    let Some(binding) = filters::get_filter_metadata_binding(&stores.storage, sheet_id, filter_id)
    else {
        return Some(filter);
    };
    ...
    match filter_kind {
        filters::FilterKind::AutoFilter => {
            ...
            for column in &imported_auto_filter.columns {
                ...
                refresh_imported_date_group_column(
                    &mut filter,
                    header_cell_id,
                    values,
                    *blanks,
...
pub(in crate::storage::engine) fn clear_imported_table_date_group_metadata_after_column_edit(
    ...
    let Some(filter) = filters::get_filter(&stores.storage, sheet_id, filter_id) else {
        return;
    };
