...
fn source_format_at(
    ...
    let cell_id = cell_store.resolve_cell_id(source_sheet_id, pos);
    let table_fmt = super::super::super::resolve_structured_format_at_cell(
        cell_store,
        source_sheet_id,
        row,
        col,
    ...
    if let Some(cell_id) = cell_id {
        ...
        properties::get_effective_format(
            ...
            cell_store.get_sheet(source_sheet_id),
        )
    } else {
...
pub(in crate::storage::engine) fn mutation_copy_range(
    ...
    // This avoids borrow conflicts when we later need &mut cell_store for
    ...
    // ── Pass 2: Process collected data with mutable access to cell_store ──
    ...
    for src in &source_data {
        match copy_type {
            CopyType::All => {
                ...
                let formula_body = render_formula_body(stores, cell_store, src);
                match formula_body {
    ...
    // Apply format edits and surface them as changed cells so mutation consumers
    ...
    for (sheet_id, row, col, format) in &format_edits {
        let Some(cell_id) = super::super::super::cell_editing::ensure_cell_id(
    ...
    let mut recalc = mutation_set_cells_by_position_raw(stores, cell_store, cell_edits, false)?;
