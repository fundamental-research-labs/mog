...
fn cell_fingerprint_for_id(
    ...
    if let Some(formula) = crate::storage::engine::formula_read::formula_text_for_cell_id(
        ...
        return fingerprint(
            ...
            &json!({
                "kind": "cellFormula",
                "formula": formula,
                "identityFormula": cell_store.get_formula(cell_id),
            }),
        );
    }
    let value = cell_store
        ...
        .and_then(|pos| cell_store.get_cell_value_at(sheet_id, pos));
    match value {
...
pub(in crate::storage::engine) fn set_cell_annotation_by_position(
    ...
    let cell_id =
        cell_editing::ensure_cell_id(stores, cell_store, sheet_id, row, col).ok_or_else(|| {
    ...
    let fingerprint = cell_fingerprint_for_id(stores, cell_store, sheet_id, &cell_id)?;
...
pub(in crate::storage::engine) fn get_cell_annotation_by_position(
    ...
    let Some(cell_id) = cell_editing::find_cell_id_at(cell_store, sheet_id, row, col) else {
        return Ok(None);
    };
    let cell_hex = id_to_hex(cell_id.as_u128()).to_string();
    let Some(record) =
    ...
    let current = Some(cell_fingerprint_for_id(
        stores, cell_store, sheet_id, &cell_id,
    )?);
...
pub(in crate::storage::engine) fn list_cell_annotations(
    ...
    sheet::annotations::list_cell_annotations(&stores.storage, sheet_id)
        ...
        .map(|record| {
            let current =
                cell_fingerprint_for_hex(stores, cell_store, sheet_id, &record.anchor_id)?;
...
fn resolve_existing_table<'a>(
    ...
    cell_store
        ...
        .or_else(|| cell_store.get_table(table_ref))
}
...
fn resolve_required_table<'a>(
    ...
    resolve_existing_table(cell_store, table_ref).ok_or_else(|| ComputeError::Eval {
        message: format!("Table not found: {}", table_ref),
    })
}

fn table_anchor_id(cell_store: &CellStore, table_ref: &str) -> String {
    resolve_existing_table(cell_store, table_ref)
...
pub(in crate::storage::engine) fn set_table_annotation(
    ...
    let table = resolve_required_table(cell_store, table_ref)?;
...
pub(in crate::storage::engine) fn get_table_annotation(
    ...
    let anchor_id = table_anchor_id(cell_store, table_ref);
    let Some(record) = workbook::annotations::get_table_annotation(&stores.storage, &anchor_id)
    else {
        return Ok(None);
    };
    let current = resolve_existing_table(cell_store, &anchor_id)
...
pub(in crate::storage::engine) fn list_table_annotations(
    ...
    workbook::annotations::list_table_annotations(&stores.storage)
        .into_iter()
        .map(|record| {
            let current = resolve_existing_table(cell_store, &record.anchor_id)
                .map(table_fingerprint)
                .transpose()?;
            Ok(validate_record(record, current))
        })
...
pub(in crate::storage::engine) fn refresh_copied_cell_annotations(
    ...
    let ids: Vec<_> = stores
        ...
        .collect();
    for id in ids {
        let fingerprint = cell_fingerprint_for_id(stores, cell_store, sheet_id, &id)?;
