...
// -------------------------------------------------------------------
// Test: CopyRange -- copy formats only
// -------------------------------------------------------------------
...
fn test_copy_range_formats_only() {
    ...
    let a5_cell_id = engine
        .stores
        .grid_indexes
        .get(&sid)
        .and_then(|g| g.cell_id_at(4, 0));
    assert!(
        a5_cell_id.is_some(),
        "A5 should have a cell_id after format copy"
    );

    let a5_hex = id_to_hex(a5_cell_id.unwrap().as_u128());
    let a5_fmt = crate::storage::properties::get_cell_format(
        engine.stores.storage.doc(),
        engine.stores.storage.workbook_map(),
        engine.stores.storage.sheets(),
        &sid,
    ...
    assert!(a5_fmt.is_some(), "A5 should have format after copy");
    assert_eq!(
        a5_fmt.unwrap().bold,
        Some(true),
    ...
    // A5 should NOT have any value (formats-only copy doesn't write values)
    let a5_val = engine
        .mirror()
        .get_cell_value_at(&sid, SheetPos::new(4, 0))
        .cloned()
...
// -------------------------------------------------------------------
// Test: CopyRange -- 'all' copies both values and formats
// -------------------------------------------------------------------
...
fn test_copy_range_all() {
    ...
    // Copy A1 (value=10, bold=true) to A5 with CopyType::All
    let _output = engine
        .apply_mutation(EngineMutation::CopyRange {
            source_sheet_id: sid,
            src_start_row: 0,
    ...
    assert!(a5_fmt.is_some(), "A5 should have format after all-copy");
    assert_eq!(
        a5_fmt.unwrap().bold,
        Some(true),
        "A5 should have bold=true copied from A1"
