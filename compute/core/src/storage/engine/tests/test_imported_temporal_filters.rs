...
fn imported_table_date_group_reapplies_after_date_system_toggle_and_refreshes_count() {
    ...
    let saved = compute_collab::encode_full_state(engine.storage().doc());
    let (reopened, _) = crate::storage::engine::YrsComputeEngine::from_yrs_state(&saved)
        .expect("reopen untouched date-group filter before explicit reapply");
...
fn user_table_filter_equal_to_stale_date_projection_is_not_recompiled() {
    ...
    let (replayed, _) = crate::storage::engine::YrsComputeEngine::from_yrs_state(&state)
        .expect("replay edited table filter");
    ...
    let cleared_state = compute_collab::encode_full_state(engine.storage().doc());
    let (cleared_replay, _) =
        crate::storage::engine::YrsComputeEngine::from_yrs_state(&cleared_state)
            .expect("replay cleared table filter");
