...
fn flag_for_position(engine: &ComputeEngine, row: u32, col: u32) -> u16 {
    engine
...
fn add_sparkline_emits_change_and_has_sparkline_patch() {
    ...
    assert_ne!(flag_for_position(&engine, 2, 3) & HAS_SPARKLINE, 0);
}
...
fn update_sparkline_move_emits_removed_and_set_changes_with_patches() {
    ...
    assert_eq!(flag_for_position(&engine, 1, 1) & HAS_SPARKLINE, 0);
    assert_ne!(flag_for_position(&engine, 4, 5) & HAS_SPARKLINE, 0);
}
...
fn delete_sparkline_emits_removed_change_and_clears_patch_flag() {
    ...
    assert_eq!(flag_for_position(&engine, 2, 2) & HAS_SPARKLINE, 0);
}
...
fn group_and_clear_sparkline_mutations_emit_member_changes() {
    ...
    assert_ne!(flag_for_position(&engine, 1, 1) & HAS_SPARKLINE, 0);
    assert_ne!(flag_for_position(&engine, 3, 3) & HAS_SPARKLINE, 0);
