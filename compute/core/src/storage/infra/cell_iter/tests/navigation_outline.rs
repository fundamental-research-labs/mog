...
fn test_find_data_edge_traverses_collapsed_outline_columns_from_hidden_detail() {
    ...
    let target = find_data_edge(&storage, sid, &grid, &cell_store, 6, 15, "right");
...
fn test_find_data_edge_traverses_collapsed_outline_columns_from_visible_boundary() {
    ...
    let target = find_data_edge(&storage, sid, &grid, &cell_store, 20, 14, "right");
...
fn test_find_data_edge_traverses_collapsed_outline_columns_from_empty_block_lead_in() {
    ...
    let target = find_data_edge(&storage, sid, &grid, &cell_store, 6, 11, "right");
...
fn test_find_data_edge_does_not_traverse_collapsed_outline_from_distant_empty_cell() {
    ...
    let target = find_data_edge(&storage, sid, &grid, &cell_store, 6, 10, "right");
...
fn test_find_data_edge_returns_visible_boundary_before_collapsed_outline_columns() {
    ...
    let target = find_data_edge(&storage, sid, &grid, &cell_store, 6, 27, "left");
...
fn test_find_data_edge_skips_hidden_columns_to_next_visible_cell() {
    ...
    let target = find_data_edge(&storage, sid, &grid, &cell_store, 6, 14, "right");
...
fn test_find_data_edge_traverses_collapsed_outline_rows_from_hidden_detail() {
    ...
    let target = find_data_edge(&storage, sid, &grid, &cell_store, 23, 5, "down");
...
fn test_find_data_edge_ignores_hidden_row_for_horizontal_navigation() {
    ...
    let left = find_data_edge(&storage, sid, &grid, &cell_store, 24, 52, "left");
    let right = find_data_edge(&storage, sid, &grid, &cell_store, 24, 52, "right");
...
fn test_find_data_edge_uses_native_occupancy_without_registered_identities() {
    let (storage, sid, grid, mut cell_store) = storage_with_grid();
    seed_cell(
        sid,
        &mut cell_store,
        0,
    ...
    let target = super::super::find_data_edge(
        &storage,
        sid,
        &grid,
        &cell_store,
