...
fn test_update_imported_note_synchronizes_runs_and_plain_text_content() {
    ...
    let persisted = get_comment(&storage, &sheet_id, &note.id).expect("updated note persists");
...
fn test_set_note_dimensions_applies_height_and_width_independently() {
    ...
    let height_only = get_comment(&storage, &sheet_id, &note.id).unwrap();
    ...
    let width_added = get_comment(&storage, &sheet_id, &note.id).unwrap();
    ...
    let unchanged = get_comment(&storage, &sheet_id, &note.id).unwrap();
...
fn test_convert_note_to_thread_flips_discriminator_and_clears_geometry() {
    ...
    set_note_visible(&mut storage, &sheet_id, &note.id, true);
    set_note_dimensions(&mut storage, &sheet_id, &note.id, Some(120.0), Some(60.0));
    // Sanity: geometry was actually written.
    let with_geometry = get_comment(&storage, &sheet_id, &note.id).unwrap();
    ...
    let converted = convert_note_to_thread(&mut storage, &sheet_id, &note.id)
    ...
    // Re-fetch and confirm the native state matches.
    let refetched = get_comment(&storage, &sheet_id, &note.id).unwrap();
...
fn test_convert_note_to_thread_preserves_note_shape_anchor() {
    ...
    insert_comment(&mut storage, &sheet_id, &stored);

    let converted = convert_note_to_thread(&mut storage, &sheet_id, &note.id).unwrap();
    assert_eq!(converted.comment_type, CommentType::ThreadedComment);
...
fn test_convert_note_to_thread_idempotent_on_existing_thread() {
    ...
    let converted = convert_note_to_thread(&mut storage, &sheet_id, &thread.id)
