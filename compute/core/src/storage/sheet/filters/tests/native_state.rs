...
#[test]
fn test_filter_native_roundtrip() {
    ...
    set_filter_sort_state(&mut storage, &sheet_id, &filter.id, Some(sort_state));

    // Read back and verify
    let fetched = get_filter(&storage, &sheet_id, &filter.id).unwrap();
