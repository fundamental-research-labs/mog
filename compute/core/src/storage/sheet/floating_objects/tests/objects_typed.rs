...
fn test_create_floating_object_and_get_typed() {
    ...
    let obj = get_floating_object_typed(&storage, &sheet_id, &object_id).unwrap();
...
fn test_update_floating_object_typed() {
    ...
    .unwrap();
    let original = get_floating_object_typed(&storage, &sheet_id, &object_id).unwrap();
    let original_updated_at = original.common.updated_at;

    let updates = serde_json::json!({ "width": 999.0, "height": 888.0 });
    let updated = update_floating_object(&mut storage, &sheet_id, &object_id, &updates);
    assert!(updated);
...
fn test_get_all_floating_objects_typed() {
    ...
    .unwrap();
    let all = get_all_floating_objects_typed(&storage, &sheet_id);
    assert_eq!(all.len(), 2);
...
fn test_opaque_set_typed_get() {
    ...
    set_floating_object(&mut storage, &sheet_id, "obj-opaque", &json).unwrap();

    // Read via typed API
    let obj = get_floating_object_typed(&storage, &sheet_id, "obj-opaque").unwrap();
