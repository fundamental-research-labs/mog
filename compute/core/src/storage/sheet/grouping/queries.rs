...
pub fn get_group(
    ...
    for sid in sheet_ids {
        if let Some(g) = get_group_in_sheet(storage, sid, group_id) {
            return Some(g);
        }
    }
    None
...
pub fn get_max_outline_level(
    ...
    get_groups(storage, sheet_id, axis)
        .iter()
        .map(|g| g.level)
        .max()
        .unwrap_or(0)
...
pub fn get_affected_rows_by_group(
    ...
    let group = match get_group_in_sheet(storage, sheet_id, group_id) {
...
pub fn get_rows_hidden_by_collapsed_groups(
    ...
    get_rows_hidden_by_structural_groups(storage, sheet_id)
}
...
pub fn get_columns_hidden_by_collapsed_groups(
    ...
    get_columns_hidden_by_structural_groups(storage, sheet_id)
}
