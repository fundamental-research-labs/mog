...
pub fn get_outline_symbols(
    ...
    for group in &get_groups(storage, sheet_id, GroupAxis::Row) {
    ...
    for group in &get_groups(storage, sheet_id, GroupAxis::Column) {
...
pub fn get_outline_level_buttons(
    ...
    let max_row = get_max_outline_level(storage, sheet_id, GroupAxis::Row);
    let max_col = get_max_outline_level(storage, sheet_id, GroupAxis::Column);
    if max_row > 0 {
...
pub fn get_outline_render_data(
    ...
    OutlineRenderData {
        config: get_sheet_grouping_config(storage, sheet_id),
        row_groups: get_groups(storage, sheet_id, GroupAxis::Row),
        column_groups: get_groups(storage, sheet_id, GroupAxis::Column),
        max_row_level: get_max_outline_level(storage, sheet_id, GroupAxis::Row),
        max_col_level: get_max_outline_level(storage, sheet_id, GroupAxis::Column),
        ...
        outline_symbols: get_outline_symbols(storage, sheet_id, viewport),
        level_buttons: get_outline_level_buttons(storage, sheet_id),
    }
}

pub fn should_render_outlines(storage: &WorkbookStorage, sheet_id: &SheetId) -> bool {
    let config = get_sheet_grouping_config(storage, sheet_id);
    if !config.show_outline_symbols {
        return false;
    }
    get_max_outline_level(storage, sheet_id, GroupAxis::Row) > 0
        || get_max_outline_level(storage, sheet_id, GroupAxis::Column) > 0
}
...
pub fn get_outline_gutter_dimensions(
    ...
    let mr = get_max_outline_level(storage, sheet_id, GroupAxis::Row);
    let mc = get_max_outline_level(storage, sheet_id, GroupAxis::Column);
    (
