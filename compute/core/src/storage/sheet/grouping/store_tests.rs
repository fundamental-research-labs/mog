...
fn test_multi_sheet_isolation() {
    ...
    group_rows(&mut s, &a, 1, 5).unwrap();
    group_rows(&mut s, &b, 10, 20).unwrap();
    group_rows(&mut s, &b, 12, 15).unwrap();
    assert_eq!(get_groups(&s, &a, GroupAxis::Row).len(), 1);
    assert_eq!(get_groups(&s, &b, GroupAxis::Row).len(), 2);
}

#[test]
fn test_nonexistent_sheet() {
    let s = WorkbookStorage::new();
    assert_eq!(
        get_sheet_grouping_config(&s, &make_sheet_id(999)),
