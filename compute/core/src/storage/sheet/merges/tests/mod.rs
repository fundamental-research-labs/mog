...
fn fresh_mirror(rows: u32, cols: u32) -> (crate::mirror::CellMirror, SheetId) {
    let id = SheetId::from_raw(1);
    let mut mirror = crate::mirror::CellMirror::new();
    mirror.add_sheet_mirror(
        id,
        "Sheet1".into(),
        crate::mirror::SheetMirror::new(id, "Sheet1".into(), rows, cols),
    );
    (mirror, id)
}
...
fn storage_with_sheet() -> (WorkbookStorage, SheetId, GridIndex) {
...
fn test_merge_range_basic() {
    ...
    assert_eq!(
        grid.cell_position(&top_left)
            .map(|(row, col)| SheetPos::new(row, col)),
        Some(SheetPos::new(0, 0))
    );
    assert_eq!(
        grid.cell_position(&bottom_right)
            .map(|(row, col)| SheetPos::new(row, col)),
...
fn test_merge_range_non_overlapping() {
    ...
    let all = get_all_merges(&storage, sid, &grid);
    assert_eq!(all.len(), 3);
}
...
fn test_unmerge_range() {
    ...
    assert_eq!(get_all_merges(&storage, sid, &grid).len(), 2);

    // Unmerge the first merge (origin at 0,0)
    let removed = unmerge_range(&mut storage, sid, &grid, 0, 0, 2, 2);
    assert_eq!(removed, 1);
    assert_eq!(get_all_merges(&storage, sid, &grid).len(), 1);

    // The remaining merge is the one at 5,5
    let remaining = get_all_merges(&storage, sid, &grid);
...
fn test_get_merge_for_cell() {
    ...
    // Origin cell
    let info = get_merge_for_cell(&storage, sid, &grid, 2, 3);
    assert!(info.is_some());
    assert!(info.unwrap().is_origin);

    // Interior cell
    let info = get_merge_for_cell(&storage, sid, &grid, 4, 5);
    assert!(info.is_some());
    assert!(!info.unwrap().is_origin);

    // Outside cell
    let info = get_merge_for_cell(&storage, sid, &grid, 0, 0);
    assert!(info.is_none());
}
...
fn test_is_merge_origin() {
    ...
    assert!(is_merge_origin(&storage, sid, &grid, 1, 1));
    assert!(!is_merge_origin(&storage, sid, &grid, 2, 2));
    assert!(!is_merge_origin(&storage, sid, &grid, 0, 0));
}
...
fn test_clear_all_merges() {
    ...
    assert_eq!(get_all_merges(&storage, sid, &grid).len(), 0);
}
...
fn test_get_merges_in_range() {
    ...
    let in_range = get_merges_in_range(&storage, sid, &grid, 0, 0, 6, 6);
    assert_eq!(in_range.len(), 2);

    // Range overlaps with only the last merge
    let in_range2 = get_merges_in_range(&storage, sid, &grid, 9, 9, 15, 15);
    assert_eq!(in_range2.len(), 1);

    // Range overlaps with nothing
    let in_range3 = get_merges_in_range(&storage, sid, &grid, 20, 20, 25, 25);
...
fn test_get_merges_in_range_viewport_intersection() {
    ...
    let in_vp = get_merges_in_range(&storage, sid, &grid, 1, 1, 10, 10);
    assert_eq!(in_vp.len(), 1);

    let in_vp_empty = get_merges_in_range(&storage, sid, &grid, 5, 5, 10, 10);
...
fn test_check_merge_data_loss_no_data() {
    let (mirror, sid) = fresh_mirror(10, 10);
    let (has_loss, count) = check_merge_data_loss(&mirror, sid, 0, 0, 1, 1);
    assert!(!has_loss);
    assert_eq!(count, 0);
}
...
fn test_validate_and_clean_merges_removes_invalid() {
    ...
    assert_eq!(get_all_merges(&storage, sid, &grid).len(), 1); // only 1 resolves

    let removed = validate_and_clean_merges(&mut storage, sid, &grid);
    assert_eq!(removed, 1);
...
fn test_unmerge_empty() {
    let (mut storage, sid, grid) = storage_with_sheet();
    let removed = unmerge_range(&mut storage, sid, &grid, 0, 0, 10, 10);
    assert_eq!(removed, 0);
}
...
fn test_unmerge_only_origin_inside() {
    ...
    let removed = unmerge_range(&mut storage, sid, &grid, 1, 0, 4, 2);
