...
#[test]
fn native_set_range_schema_creates_validation_ranges() {
    let (mut storage, sid, _gi) = storage_with_sheet();
    let rs = make_range_schema("rs-5d-1");
    set_range_schema(&mut storage, &sid, &rs).unwrap();

    // Native validation store has the entry.
    assert_eq!(validation_rule_count(&storage, &sid), 1);
    let fetched = get_range_schema(&storage, &sid, "rs-5d-1").expect("rs-5d-1");
    assert_eq!(fetched.id, "rs-5d-1");
}
#[test]
fn native_set_range_schema_clears_imported_declared_count_metadata() {
    let (mut storage, sid, _gi) = storage_with_sheet();
    storage
        .sheet_metadata
        .get_mut(&sid)
...
#[test]
fn native_delete_range_schema_cleans_up_ranges() {
    let (mut storage, sid, _gi) = storage_with_sheet();
    let rs = make_range_schema("rs-5d-del");
    set_range_schema(&mut storage, &sid, &rs).unwrap();

    assert_eq!(validation_rule_count(&storage, &sid), 1);
    delete_range_schema(&mut storage, &sid, "rs-5d-del");

    // Native validation store cleaned up.
    assert_eq!(validation_rule_count(&storage, &sid), 0);
    // View layer confirms deletion.
    assert!(get_range_schema(&storage, &sid, "rs-5d-del").is_none());
}
#[test]
fn native_delete_range_schema_clears_imported_declared_count_metadata() {
    let (mut storage, sid, _gi) = storage_with_sheet();
    ...
    delete_range_schema(&mut storage, &sid, "rs-5d-del-count");
...
#[test]
fn native_multiple_range_schemas_independent_delete() {
    let (mut storage, sid, _gi) = storage_with_sheet();
    let rs1 = make_range_schema("rs-5d-a");
    let rs2 = make_range_schema("rs-5d-b");
    set_range_schema(&mut storage, &sid, &rs1).unwrap();
    set_range_schema(&mut storage, &sid, &rs2).unwrap();

    assert_eq!(validation_rule_count(&storage, &sid), 2);

    // Delete one, keep the other.
    delete_range_schema(&mut storage, &sid, "rs-5d-a");
    assert_eq!(validation_rule_count(&storage, &sid), 1);
    assert!(get_range_schema(&storage, &sid, "rs-5d-a").is_none());
    assert!(get_range_schema(&storage, &sid, "rs-5d-b").is_some());
}
#[test]
fn native_update_range_schema_replaces_ranges() {
    let (mut storage, sid, _gi) = storage_with_sheet();
    ...
    update_range_schema(&mut storage, &sid, "rs-5d-upd", &updated).unwrap();

    // Still only one entry in the store.
    assert_eq!(validation_rule_count(&storage, &sid), 1);

    // Updated field is reflected.
    let fetched = get_range_schema(&storage, &sid, "rs-5d-upd").expect("rs-5d-upd");
    assert_eq!(fetched.enforcement, Some(EnforcementLevel::Warning));
}
#[test]
fn native_single_cell_validation() {
...
#[test]
fn native_first_match_semantics() {
