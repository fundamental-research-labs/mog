...
fn test_set_and_get_range_schema() {
    ...
    let fetched = get_range_schema(&storage, &sid, "rs-1").expect("rs-1");
...
fn test_get_range_schema_missing() {
    ...
    assert!(get_range_schema(&storage, &sid, "nope").is_none());
}
#[test]
fn test_get_range_schemas_for_sheet() {
    ...
    let all = get_range_schemas_for_sheet(&storage, &sid);
    assert_eq!(all.len(), 2);
...
fn test_update_range_schema() {
    ...
    update_range_schema(&mut storage, &sid, "rs-upd", &updated).unwrap();

    let fetched = get_range_schema(&storage, &sid, "rs-upd").expect("present");
...
fn test_delete_range_schema() {
    ...
    set_range_schema(&mut storage, &sid, &rs).unwrap();
    assert!(get_range_schema(&storage, &sid, "rs-del").is_some());

    delete_range_schema(&mut storage, &sid, "rs-del");
    assert!(get_range_schema(&storage, &sid, "rs-del").is_none());
...
fn test_delete_range_schema_noop() {
    let (mut storage, sid, _gi) = storage_with_sheet();
    // Should not panic
    delete_range_schema(&mut storage, &sid, "nonexistent");
}
#[test]
fn test_set_range_schema_nonexistent_sheet() {
    ...
    let result = set_range_schema(&mut storage, &sid, &rs);
...
fn test_update_range_schema_nonexistent_creates() {
    ...
    update_range_schema(&mut storage, &sid, "rs-new", &rs).unwrap();

    let fetched = get_range_schema(&storage, &sid, "rs-new");
...
fn test_multiple_range_schemas_yield_n_entries_not_2n() {
    ...
    let n = 5;
    for i in 0..n {
        set_range_schema(&mut storage, &sid, &make_range_schema(&format!("rs-{i}"))).unwrap();
    }
    assert_eq!(validation_rule_count(&storage, &sid), n);
    // View layer also reports exactly n entries.
    let view = get_range_schemas_for_sheet(&storage, &sid);
