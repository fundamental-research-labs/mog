...
fn test_add_sheet() {
    ...
    storage
        .add_sheet(&mut cell_store, sheet_id, "MySheet", 100, 26)
    ...
    assert!(cell_store.sheet_by_name("mysheet").is_some());
...
fn test_add_multiple_sheets_order_preserved() {
    ...
    storage
        .add_sheet(&mut cell_store, s1, "First", 10, 5)
        .unwrap();
    storage
        .add_sheet(&mut cell_store, s2, "Second", 10, 5)
        .unwrap();
    storage
        .add_sheet(&mut cell_store, s3, "Third", 10, 5)
        .unwrap();
...
fn test_remove_sheet() {
    ...
    storage
        .add_sheet(&mut cell_store, s1, "Sheet1", 10, 5)
        .unwrap();
    storage
        .add_sheet(&mut cell_store, s2, "Sheet2", 10, 5)
        .unwrap();
    assert_eq!(storage.sheet_order().len(), 2);

    storage.remove_sheet(&mut cell_store, &s1);
    ...
    assert!(cell_store.sheet_by_name("sheet2").is_some());
...
fn test_remove_nonexistent_sheet() {
    let mut storage = WorkbookStorage::new();
    let mut cell_store = CellStore::new();
    storage.remove_sheet(&mut cell_store, &make_sheet_id(999));
