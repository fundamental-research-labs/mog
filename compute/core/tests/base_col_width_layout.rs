...
fn import_directly(xlsx: &[u8], mdw: f64) -> ComputeEngine {
    let (mut engine, _) =
        ComputeEngine::from_snapshot_with_layout_metrics(WorkbookSnapshot::default(), metrics(mdw))
...
fn base_col_width_drives_layout_and_viewport_for_mdw_profiles() {
    ...
    for (mdw, expected_width) in [(7.0, 75.0), (8.0, 85.0)] {
        ...
        // The native rebuild uses round-trip sheet metadata, so the same
...
#[test]
fn base_col_width_survives_xlsx_reload_and_deferred_hydration() {
    let xlsx = base_only_xlsx();
    ...
    for (mdw, expected_width) in [(7.0, 75.0), (8.0, 85.0)] {
        ...
        let state = source
            ...
            .expect("export workbook for reload");
        let mut reloaded = import_directly(&state, mdw);
        ...
        let (mut deferred, _) = ComputeEngine::from_snapshot_with_layout_metrics(
        ...
        deferred
            ...
            .expect("deferred hydration should commit native storage");
        assert_layout_and_viewport_width(&deferred, expected_width);
    }
}
...
fn explicit_default_col_width_wins_over_base_and_missing_values_use_profile_fallback() {
    for (mdw, explicit_width, fallback_width) in [(7.0, 70.0, 64.0), (8.0, 79.0, 72.0)] {
        ...
        // Exercise the parse-output index builder as well as the native metadata
