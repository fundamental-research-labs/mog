...
fn cache_sparse_override_edit() {
    ...
    assert_dense_retained(scenario, &cell_store, 1);

    let v0_before = col_version(&cell_store, 0);
    let v1_before = col_version(&cell_store, 1);

    let sid = cell_store.sheet_by_name("sheet1").unwrap();
    let cell_id = cell_types::CellId::from_uuid_str(&cell_uuid(2, 0)).unwrap();
    let result = core
        .set_cell(&mut cell_store, &sid, cell_id, 2, 0, "100")
        .expect("set_cell failed");

    assert_col_version_bumped(scenario, 0, v0_before, col_version(&cell_store, 0));
    assert_col_version_unchanged(scenario, 1, v1_before, col_version(&cell_store, 1));
    assert_col_value(scenario, &cell_store, 2, 0, number(100.0));
...
fn cache_payload_replacement() {
    ...
    let result = core
        .apply_changes(&mut cell_store, &edits, false)
        .expect("apply_changes failed");

    assert_col_version_bumped(scenario, 0, v0_before, col_version(&cell_store, 0));
    assert_col_version_bumped(scenario, 1, v1_before, col_version(&cell_store, 1));
    assert_col_value(scenario, &cell_store, 0, 0, number(10.0));
    assert_col_value(scenario, &cell_store, 4, 0, number(50.0));
    assert_col_value(scenario, &cell_store, 2, 1, number(300.0));
    assert_dense_invalidated(scenario, &cell_store, 0);
...
fn cache_compaction() {
    ...
    let _ = core
        .set_cell(&mut cell_store, &sid, cell_a1, 0, 0, "10")
        .unwrap();
    let _ = core
        .set_cell(&mut cell_store, &sid, cell_a2, 1, 0, "20")
        .unwrap();

    assert_col_value(scenario, &cell_store, 0, 0, number(10.0));
    assert_col_value(scenario, &cell_store, 1, 0, number(20.0));
    ...
    assert_col_value(scenario, &cell_store, 0, 0, number(10.0));
    assert_col_value(scenario, &cell_store, 1, 0, number(20.0));
    assert_col_value(scenario, &cell_store, 2, 0, number(3.0));
    assert_dense_invalidated(scenario, &cell_store, 0);

    let sum_cell_id = cell_types::CellId::from_uuid_str(&cell_uuid(0, 2)).unwrap();
    let sum_val = cell_store.get_cell_value(&sum_cell_id);
    match sum_val {
    ...
    assert_eq!(col_data_value(&cell_store, 0, 2), number(3.0));
}
