...
fn test_sequence_spill_with_sum_interactive() {
    ...
    let result = core
        .set_cell(&mut cell_store, &sheet_id, a1_id, 0, 0, "=SEQUENCE(10)")
        .expect("set A1");
    println!(
    ...
    assert_store_number(&cell_store, &a1_id, 1.0, "A1 after SEQUENCE(10)");
    ...
    let _r2 = core
        .set_cell(&mut cell_store, &sheet_id, b1_id, 0, 1, "=SUM(A1:A10)")
        .expect("set B1");
    assert_store_number(&cell_store, &b1_id, 55.0, "B1 SUM(A1:A10)");
}
...
fn test_sequence_spill_partial_recalc_updates_sum() {
    ...
    core.set_cell(&mut cell_store, &sheet_id, a1_id, 0, 0, "=SEQUENCE(5)")
        .expect("set A1");
    core.set_cell(&mut cell_store, &sheet_id, b1_id, 0, 1, "=SUM(A1:A5)")
        .expect("set B1");
    assert_store_number(&cell_store, &b1_id, 15.0, "B1 initial SUM");
    ...
    .expect("change A1");
    assert_store_number(&cell_store, &a1_id, 10.0, "A1 after change");
    for row in 1..5u32 {
    ...
    assert_store_number(&cell_store, &b1_id, 150.0, "B1 after SEQUENCE change");
}
...
fn test_spill_and_sum_in_initial_snapshot() {
    ...
    let a1_id = CellId::from_uuid_str(&cell_uuid(0, 0, 0)).expect("a1");
    let b1_id = CellId::from_uuid_str(&cell_uuid(0, 0, 1)).expect("b1");
    assert_store_number(&cell_store, &a1_id, 1.0, "A1 SEQUENCE source");
    let sheet_id = SheetId::from_uuid_str(&sheet_uuid(0)).expect("sid");
    // Check projected values via col_data (no phantom CellIds)
    for row in 1..10u32 {
        assert_col_data_number(
    ...
    assert_store_number(&cell_store, &b1_id, 55.0, "B1 SUM(A1:A10) over spill");
}
...
fn test_multiple_aggregates_over_spill_column() {
    ...
    core.set_cell(&mut cell_store, &sid, a1, 0, 0, "=SEQUENCE(10)")
        .expect("set A1");
    core.set_cell(&mut cell_store, &sid, b1, 0, 1, "=SUM(A1:A10)")
        .expect("set B1");
    core.set_cell(&mut cell_store, &sid, b2, 1, 1, "=AVERAGE(A1:A10)")
        .expect("set B2");
    core.set_cell(&mut cell_store, &sid, b3, 2, 1, "=COUNT(A1:A10)")
        .expect("set B3");
    core.set_cell(&mut cell_store, &sid, b4, 3, 1, "=MIN(A1:A10)")
        .expect("set B4");
    core.set_cell(&mut cell_store, &sid, b5, 4, 1, "=MAX(A1:A10)")
        .expect("set B5");
    assert_store_number(&cell_store, &b1, 55.0, "SUM");
    assert_store_number(&cell_store, &b2, 5.5, "AVERAGE");
    assert_store_number(&cell_store, &b3, 10.0, "COUNT");
    assert_store_number(&cell_store, &b4, 1.0, "MIN");
    assert_store_number(&cell_store, &b5, 10.0, "MAX");
}
...
fn test_2d_spill_sum_on_phantom_column() {
    ...
    core.set_cell(&mut cell_store, &sid, a1, 0, 0, "=SEQUENCE(5,3)")
        .expect("set A1");
    assert_store_number(&cell_store, &a1, 1.0, "A1 source");
    // Check projected values via col_data (no phantom CellIds)
    let eb = [2.0, 5.0, 8.0, 11.0, 14.0];
    for (row, exp) in eb.iter().enumerate() {
        let r = row as u32;
        assert_col_data_number(&cell_store, &sid, r, 1, *exp, &format!("B{}", r + 1));
    }
    let ec = [3.0, 6.0, 9.0, 12.0, 15.0];
    for (row, exp) in ec.iter().enumerate() {
        let r = row as u32;
        assert_col_data_number(&cell_store, &sid, r, 2, *exp, &format!("C{}", r + 1));
    }
    core.set_cell(&mut cell_store, &sid, d1, 0, 3, "=SUM(B1:B5)")
        .expect("set D1");
    core.set_cell(&mut cell_store, &sid, d2, 1, 3, "=SUM(C1:C5)")
        .expect("set D2");
    assert_store_number(&cell_store, &d1, 40.0, "D1 SUM(B1:B5)");
    assert_store_number(&cell_store, &d2, 45.0, "D2 SUM(C1:C5)");
}
...
fn test_spill_shrinkage_updates_sum() {
    ...
    core.set_cell(&mut cell_store, &sid, b1, 0, 1, "=SUM(A1:A10)")
        .expect("set B1");
    assert_store_number(&cell_store, &b1, 55.0, "B1 initial");
    ...
    assert_store_number(&cell_store, &b1, 15.0, "B1 after shrinkage");
}
...
fn test_spill_expansion_updates_sum() {
    ...
    core.set_cell(&mut cell_store, &sid, b1, 0, 1, "=SUM(A1:A10)")
        .expect("set B1");
    assert_store_number(&cell_store, &b1, 15.0, "B1 initial partial");
    // Expand to SEQUENCE(10)
    core.set_cell(&mut cell_store, &sid, a1, 0, 0, "=SEQUENCE(10)")
        .expect("expand A1");
    assert_store_number(&cell_store, &a1, 1.0, "A1 after expand");
    // Check projected values via col_data (no phantom CellIds)
    for row in 1..10u32 {
        assert_col_data_number(
            &cell_store,
    ...
    assert_store_number(&cell_store, &b1, 55.0, "B1 after expansion");
}
...
fn test_dependent_on_individual_phantom() {
    ...
    core.set_cell(&mut cell_store, &sid, a1, 0, 0, "=SEQUENCE(5)")
        .expect("set A1");
    core.set_cell(&mut cell_store, &sid, b3, 2, 1, "=A3*10")
        .expect("set B3");
    assert_store_number(&cell_store, &b3, 30.0, "B3 = A3*10");
    // Change: A3 becomes 300
    core.set_cell(&mut cell_store, &sid, a1, 0, 0, "=SEQUENCE(5,1,100,100)")
        .expect("change A1");
    assert_store_number(&cell_store, &b3, 3000.0, "B3 after change");
}
...
fn test_spill_b2_depends_on_a2_interactive() {
    ...
    core.set_cell(&mut cell_store, &sid, a1, 0, 0, "=SEQUENCE(2)")
        .expect("set A1");
    assert_store_number(&cell_store, &a1, 1.0, "A1 source");
    // Verify A2 projected value via col_data (no phantom CellIds)
    assert_col_data_number(&cell_store, &sid, 1, 0, 2.0, "A2 projected");
    // B2 = A2 → should read spilled value 2
    core.set_cell(&mut cell_store, &sid, b2, 1, 1, "=A2")
        .expect("set B2");
    assert_store_number(&cell_store, &b2, 2.0, "B2 = A2 (spilled value)");
    // Also test B2 = A2 + 1
    core.set_cell(&mut cell_store, &sid, b2, 1, 1, "=A2+1")
        .expect("set B2 to A2+1");
    assert_store_number(&cell_store, &b2, 3.0, "B2 = A2+1 (spilled value + 1)");
}
...
fn test_spill_b2_depends_on_a2_initial_snapshot() {
    ...
    let a1 = CellId::from_uuid_str(&cell_uuid(0, 0, 0)).expect("a1");
    let b2 = CellId::from_uuid_str(&cell_uuid(0, 1, 1)).expect("b2");
    assert_store_number(&cell_store, &a1, 1.0, "A1 SEQUENCE source");
    // B2 = A2 + 1 = 2 + 1 = 3
    assert_store_number(&cell_store, &b2, 3.0, "B2 = A2+1 over spilled phantom");
}
...
fn test_mixed_data_and_spill_sum() {
    ...
    let b1 = CellId::from_uuid_str(&cell_uuid(0, 0, 1)).expect("b1");
    core.set_cell(&mut cell_store, &sid, a6, 5, 0, "=SEQUENCE(5)")
        .expect("set A6");
    core.set_cell(&mut cell_store, &sid, b1, 0, 1, "=SUM(A1:A10)")
        .expect("set B1");
