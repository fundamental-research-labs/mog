...
fn test_transpose_horizontal_spill_interactive() {
    ...
    core.set_cell(&mut cell_store, &sid, b1, 0, 1, "=TRANSPOSE(A1:A5)")
        .expect("set B1");
    ...
    assert_col_data_number(&cell_store, &sid, 0, 2, 20.0, "C1 spill target");
    assert_col_data_number(&cell_store, &sid, 0, 3, 30.0, "D1 spill target");
    assert_col_data_number(&cell_store, &sid, 0, 4, 40.0, "E1 spill target");
    assert_col_data_number(&cell_store, &sid, 0, 5, 50.0, "F1 spill target");
}
...
fn test_transpose_vertical_spill_from_horizontal_input() {
    ...
    core.set_cell(&mut cell_store, &sid, f2, 1, 5, "=TRANSPOSE(A1:E1)")
        .expect("set F2");

    // Source cell = first transposed value (10)
    assert_store_number(&cell_store, &f2, 10.0, "F2 TRANSPOSE source");

    // Vertical spill: F3=20, F4=30, F5=40, F6=50 (all in column 5)
    assert_col_data_number(&cell_store, &sid, 2, 5, 20.0, "F3 vertical spill");
    assert_col_data_number(&cell_store, &sid, 3, 5, 30.0, "F4 vertical spill");
    assert_col_data_number(&cell_store, &sid, 4, 5, 40.0, "F5 vertical spill");
    assert_col_data_number(&cell_store, &sid, 5, 5, 50.0, "F6 vertical spill");
}
...
fn test_transpose_source_col_data_is_scalar() {
    ...
    assert_store_number(&cell_store, &b1, 10.0, "B1 get_cell_value");

    // col_data[B1] should be scalar Number(10), NOT CellValue::Array
    let sheet_store = &cell_store.get_sheet(&sid).expect("sheet");
    let col_b = cell_store.get_column_view(&sheet_store.id, 1).expect("col_data for B");
    match &col_b[0] {
...
fn test_transpose_spill_target_read_by_simple_formula() {
    ...
    core.set_cell(&mut cell_store, &sid, g1, 0, 6, "=C1+D1")
        .expect("set G1");

    // C1+D1 = 20+30 = 50
    assert_store_number(&cell_store, &g1, 50.0, "G1 = C1+D1 (spill target reads)");
}
...
fn test_transpose_small_spill_with_sum_interactive() {
    ...
    core.set_cell(&mut cell_store, &sid, b1, 0, 1, "=TRANSPOSE(A1:A2)")
        .expect("set B1");
    core.set_cell(&mut cell_store, &sid, d1, 0, 3, "=SUM(B1:C1)")
        .expect("set D1");

    // SUM(B1:C1) = 10+20 = 30
    assert_store_number(&cell_store, &d1, 30.0, "D1 SUM(B1:C1) small spill");
}
...
fn test_transpose_spill_with_sum_interactive() {
    ...
    assert_store_number(&cell_store, &g1, 150.0, "G1 SUM over TRANSPOSE spill");
}
...
fn test_transpose_spill_with_sum_batch() {
    ...
    assert_store_number(&cell_store, &b1, 10.0, "B1 TRANSPOSE source (batch)");
    assert_store_number(
        &cell_store,
        &g1,
        150.0,
        "G1 SUM over TRANSPOSE spill (batch)",
    );
}
...
fn test_transpose_spill_sum_single_column_range() {
    ...
    core.set_cell(&mut cell_store, &sid, e1, 0, 4, "=SUM(B1:B3)")
        .expect("set E1");
...
fn test_transpose_spill_with_array_ref_preregistration() {
    ...
    assert_store_number(&cell_store, &b1, 10.0, "B1 TRANSPOSE source (pre-reg)");
    ...
    assert_col_data_number(&cell_store, &sid, 0, 2, 20.0, "C1 spill (pre-reg)");
    assert_col_data_number(&cell_store, &sid, 0, 3, 30.0, "D1 spill (pre-reg)");
    assert_col_data_number(&cell_store, &sid, 0, 4, 40.0, "E1 spill (pre-reg)");
    assert_col_data_number(&cell_store, &sid, 0, 5, 50.0, "F1 spill (pre-reg)");

    // SUM must read the spilled values correctly
    assert_store_number(&cell_store, &g1, 150.0, "G1 SUM (pre-reg path)");
}
