...
fn assert_pos_bool(cell_store: &CellStore, row: u32, col: u32, expected: bool) {
    match pos_value(cell_store, row, col) {
...
fn regex_parsed_formula_smoke_cases() {
    ...
    assert_pos_bool(&cell_store, 0, 0, true);
    ...
    assert_pos_bool(&cell_store, 1, 0, true);
    ...
    assert_pos_bool(&cell_store, 2, 0, true);
    ...
    assert_pos_text(&cell_store, 3, 0, "ax bx cx");
    ...
    assert_pos_text(&cell_store, 4, 0, "A12");
}
...
fn regexextract_all_matches_spills_vertically() {
    ...
    assert_pos_text(&cell_store, 0, 0, "A12");
    assert_pos_text(&cell_store, 1, 0, "B345");
    assert_pos_null(&cell_store, 0, 2, 0);
}
...
fn regexextract_captures_spill_horizontally() {
    ...
    assert_pos_text(&cell_store, 0, 0, "alice");
    assert_pos_text(&cell_store, 0, 1, "42");
    assert_pos_null(&cell_store, 0, 0, 2);
}
...
fn regexextract_spill_consumers_and_scalar_context() {
    ...
    assert_pos_number(&cell_store, 0, 0, 0, 3.0);
    ...
    assert_pos_text(&cell_store, 1, 0, "a1,b2");
    ...
    assert_pos_text(&cell_store, 3, 0, "123!");
}

#[test]
fn regexextract_array_input_nested_spill_is_value_error() {
    let (mut core, mut cell_store) = init_empty();
    let _ = set(&mut core, &mut cell_store, 0, 0, 0, "a1 b2");
    let _ = set(
        &mut core,
        &mut cell_store,
        0,
    ...
    assert_pos_error(&cell_store, 1, 0, CellError::Value);
}
