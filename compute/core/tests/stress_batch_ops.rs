...
fn test_apply_changes_creates_cycle() {
    ...
    let result = core.apply_changes(&mut cell_store, &edits, true).unwrap();
...
fn test_apply_changes_breaks_cycle() {
    ...
    let init_result = core.init_from_snapshot(&mut cell_store, snapshot).unwrap();
    assert!(
    ...
    let _result = core.apply_changes(&mut cell_store, &edits, false).unwrap();
...
fn test_skip_cycle_check_comparison() {
    ...
    let mut mirror1 = CellStore::default();
    ...
    let mut mirror2 = CellStore::default();
    ...
    assert_store_error(&mirror2, 0, 0, 0, CellError::Circ);
    assert_store_error(&mirror2, 0, 0, 1, CellError::Circ);
}
...
fn test_interleaved_set_cell_and_apply_changes() {
    // --- Path A: skip=false → B1=#REF! ---
    {
        ...
        let _r2 = core.apply_changes(&mut cell_store, &edits, false).unwrap();
    ...
    // --- Path B: skip=true → mutation-created cycle materializes as #CIRC ---
    {
        ...
        let r2 = core.apply_changes(&mut cell_store, &edits, true).unwrap();
...
fn test_undo_via_apply_changes() {
    ...
    assert!(is_ref_error(&cell_store, 0, 0, 1), "B1 should be #REF!");

    // Undo: revert B1 to Null via apply_changes
    let edits = vec![make_edit(0, 0, 1, CellValue::Null, None)];
    let _r3 = core.apply_changes(&mut cell_store, &edits, false).unwrap();

    // B1 is now Null (empty), A1="=B1+1" → 0+1=1
    assert_store_null(&cell_store, 0, 0, 1);
    assert_store_number(&cell_store, 0, 0, 0, 1.0);
}
...
fn test_redo_via_apply_changes() {
    ...
    // B1="=A1+1" → #REF!
    let _r2 = set(&mut core, &mut cell_store, 0, 0, 1, "=A1+1");
    assert!(is_ref_error(&cell_store, 0, 0, 1));

    // Undo: revert B1
    let undo_edits = vec![make_edit(0, 0, 1, CellValue::Null, None)];
    let _r3 = core
        .apply_changes(&mut cell_store, &undo_edits, false)
        .unwrap();
    assert_store_null(&cell_store, 0, 0, 1);
    assert_store_number(&cell_store, 0, 0, 0, 1.0);
    ...
    let r4 = core
        .apply_changes(&mut cell_store, &redo_edits, true)
        .unwrap();
...
fn test_large_batch_50_edits() {
    ...
    assert_store_number(&cell_store, 0, 0, 2, 1.0); // C1=1
    assert_store_number(&cell_store, 0, 9, 2, 10.0); // C10=10
    assert_store_number(&cell_store, 0, 47, 2, 48.0); // C48=48
...
fn test_mixed_formulas_and_values_no_cycle() {
    ...
    let result = core.apply_changes(&mut cell_store, &edits, false).unwrap();

    assert_store_number(&cell_store, 0, 0, 1, 100.0); // B1=100
    assert_store_number(&cell_store, 0, 0, 0, 101.0); // A1="=B1+1"=101
    assert!(
        !has_any_circular_error(&result),
