...
fn test_build_three_statement_model() {
    ...
    set(&mut core, &mut cell_store, 0, 0, 0, "1000");
    assert_store_number(&cell_store, 0, 0, 0, 1000.0);

    // Step 2: COGS A2 = "=A1*0.4"
    set(&mut core, &mut cell_store, 0, 1, 0, "=A1*0.4");
    assert_store_number(&cell_store, 0, 1, 0, 400.0);

    // Step 3: Gross Profit A3 = "=A1-A2"
    set(&mut core, &mut cell_store, 0, 2, 0, "=A1-A2");
    assert_store_number(&cell_store, 0, 2, 0, 600.0);

    // Step 4: OpEx A4 = 200
    set(&mut core, &mut cell_store, 0, 3, 0, "200");
    assert_store_number(&cell_store, 0, 3, 0, 200.0);

    // Step 5: EBIT (non-circular first) A5 = "=A3-A4" = 600-200 = 400
    set(&mut core, &mut cell_store, 0, 4, 0, "=A3-A4");
    assert_store_number(&cell_store, 0, 4, 0, 400.0);

    // Step 6: Tax A6 = "=A5*0.3" = 400*0.3 = 120 (no cycle yet)
    set(&mut core, &mut cell_store, 0, 5, 0, "=A5*0.3");
    assert_store_number(&cell_store, 0, 5, 0, 120.0);

    // Step 7: NetIncome A7 = "=A5-A6" = 400-120 = 280
    set(&mut core, &mut cell_store, 0, 6, 0, "=A5-A6");
    assert_store_number(&cell_store, 0, 6, 0, 280.0);
    ...
    let _r = core.set_cells(&mut cell_store, &edits, true).unwrap();
    ...
    assert_store_number_tol(&cell_store, 0, 4, 0, expected_pretax, 0.01); // A5 PreTax
    assert_store_number_tol(&cell_store, 0, 5, 0, expected_tax, 0.01); // A6 Tax
    assert_store_number_tol(&cell_store, 0, 6, 0, expected_ni, 0.01); // A7 NetIncome

    // Non-circular cells should be unchanged
    assert_store_number(&cell_store, 0, 0, 0, 1000.0); // Revenue
    assert_store_number(&cell_store, 0, 1, 0, 400.0); // COGS
    assert_store_number(&cell_store, 0, 2, 0, 600.0); // Gross
    assert_store_number(&cell_store, 0, 3, 0, 200.0); // OpEx

    // --- Steps 9-15: Add balance sheet items ---

    // Step 9: Balance!A1 = Total Assets = 5000
    set(&mut core, &mut cell_store, 1, 0, 0, "5000");
    assert_store_number(&cell_store, 1, 0, 0, 5000.0);
    ...
    set(&mut core, &mut cell_store, 1, 2, 0, "2000");
    assert_store_number(&cell_store, 1, 2, 0, 2000.0);

    // Step 10b: Balance!A2 = Equity = "=A1-A3" = 5000-2000 = 3000
    set(&mut core, &mut cell_store, 1, 1, 0, "=A1-A3");
    assert_store_number(&cell_store, 1, 1, 0, 3000.0);

    // --- Steps 16-25: Add CashFlow items ---

    // Step 16: CashFlow!A1 = Operating CF = "=Income!A7" (NetIncome from income sheet)
    set(&mut core, &mut cell_store, 2, 0, 0, "=Income!A7");
    assert_store_number_tol(&cell_store, 2, 0, 0, expected_ni, 0.01);

    // Step 17: CashFlow!A2 = CapEx = 100
    set(&mut core, &mut cell_store, 2, 1, 0, "100");
    assert_store_number(&cell_store, 2, 1, 0, 100.0);

    // Step 18: CashFlow!A3 = Free CF = "=A1-A2"
    set(&mut core, &mut cell_store, 2, 2, 0, "=A1-A2");
    assert_store_number_tol(&cell_store, 2, 2, 0, expected_ni - 100.0, 0.01);

    // --- Steps 26-35: Modify Revenue to 2000, verify cascade ---

    // Step 26: Change Revenue to 2000
    set(&mut core, &mut cell_store, 0, 0, 0, "2000");
    assert_store_number(&cell_store, 0, 0, 0, 2000.0);

    // Step 27: COGS cascades = 2000*0.4 = 800
    assert_store_number(&cell_store, 0, 1, 0, 800.0);

    // Step 28: Gross = 2000-800 = 1200
    assert_store_number(&cell_store, 0, 2, 0, 1200.0);
    ...
    assert_store_number_tol(&cell_store, 0, 4, 0, expected_pretax2, 0.01); // PreTax
    assert_store_number_tol(&cell_store, 0, 5, 0, expected_tax2, 0.01); // Tax
    assert_store_number_tol(&cell_store, 0, 6, 0, expected_ni2, 0.01); // NetIncome

    // Step 30: CashFlow!A1 cascades = NetIncome2
    assert_store_number_tol(&cell_store, 2, 0, 0, expected_ni2, 0.01);

    // Step 31: Free CF = NI2 - 100
    assert_store_number_tol(&cell_store, 2, 2, 0, expected_ni2 - 100.0, 0.01);

    // --- Steps 36-40: Change tax rate by modifying Tax formula ---

    // Step 36: Add tax rate cell B1 = 0.25 on Income sheet
    set(&mut core, &mut cell_store, 0, 0, 1, "0.25");
    assert_store_number(&cell_store, 0, 0, 1, 0.25);
    ...
    let _r = core.set_cells(&mut cell_store, &edits2, true).unwrap();

    // New closed form with rate=0.25:
    // PreTax = 1000-Tax, Tax = PreTax*0.25
    // 1.25*Tax = 250 → Tax = 200
    // PreTax = 800, NI = 600
    assert_store_number_tol(&cell_store, 0, 5, 0, 200.0, 0.01); // Tax
    assert_store_number_tol(&cell_store, 0, 4, 0, 800.0, 0.01); // PreTax
    assert_store_number_tol(&cell_store, 0, 6, 0, 600.0, 0.01); // NetIncome

    // Step 38: CashFlow cascades
    assert_store_number_tol(&cell_store, 2, 0, 0, 600.0, 0.01); // Operating CF
    assert_store_number_tol(&cell_store, 2, 2, 0, 500.0, 0.01); // Free CF = 600-100
}
...
fn test_modify_assumptions_20_sweeps() {
    ...
    assert_store_number(&cell_store, 0, 0, 0, 1000.0); // Revenue
    assert_store_number(&cell_store, 0, 1, 0, 600.0); // GrossProfit
    ...
    assert_store_number_tol(&cell_store, 0, 2, 0, pt0, 0.01); // PreTax
    assert_store_number_tol(&cell_store, 0, 3, 0, tax0, 0.01); // Tax

    // Add NetIncome after cycle is established
    set(&mut core, &mut cell_store, 0, 4, 0, "=A3-A4");
    let ni0 = pt0 - tax0;
    assert_store_number_tol(&cell_store, 0, 4, 0, ni0, 0.01); // NetIncome
    ...
    for step in 1u32..=20 {
        ...
        assert_store_number(&cell_store, 0, 0, 0, revenue);
        assert_store_number(&cell_store, 0, 1, 0, gp);
        assert_store_number_tol(&cell_store, 0, 2, 0, pretax, 0.01);
        assert_store_number_tol(&cell_store, 0, 3, 0, tax, 0.01);
        assert_store_number_tol(&cell_store, 0, 4, 0, ni, 0.01);
    }
}
...
fn test_scenario_analysis_three_columns() {
    ...
    let build_scenario =
        |core: &mut ComputeCore, cell_store: &mut CellStore, col: u32, revenue: f64| {
            // Set revenue constant first
            let col_letter = (b'A' + col as u8) as char;
            let rev_str = format!("{}", revenue as u32);

            // Row 0: Revenue
            set(core, cell_store, 0, 0, col, &rev_str);
            ...
            core.set_cells(cell_store, &edits, true).unwrap();
        };

    // Step 1-5: Build Base scenario (column A, col=0)
    build_scenario(&mut core, &mut cell_store, 0, 1000.0);
    ...
    assert_store_number(&cell_store, 0, 0, 0, 1000.0);
    assert_store_number(&cell_store, 0, 1, 0, 600.0);
    assert_store_number_tol(&cell_store, 0, 2, 0, base_pt, 0.01);
    assert_store_number_tol(&cell_store, 0, 3, 0, base_tax, 0.01);
    assert_store_number_tol(&cell_store, 0, 4, 0, base_ni, 0.01);

    // Step 6-10: Build Bull scenario (column B, col=1)
    build_scenario(&mut core, &mut cell_store, 1, 2000.0);

    let bull_pt = 2000.0 * 0.6 / 1.3;
    let bull_tax = bull_pt * 0.3;
    let bull_ni = bull_pt - bull_tax;
    assert_store_number(&cell_store, 0, 0, 1, 2000.0);
    assert_store_number(&cell_store, 0, 1, 1, 1200.0);
    assert_store_number_tol(&cell_store, 0, 2, 1, bull_pt, 0.01);
    assert_store_number_tol(&cell_store, 0, 3, 1, bull_tax, 0.01);
    assert_store_number_tol(&cell_store, 0, 4, 1, bull_ni, 0.01);
    ...
    build_scenario(&mut core, &mut cell_store, 2, 500.0);

    let bear_pt = 500.0 * 0.6 / 1.3;
    let bear_tax = bear_pt * 0.3;
    let bear_ni = bear_pt - bear_tax;
    assert_store_number(&cell_store, 0, 0, 2, 500.0);
    assert_store_number(&cell_store, 0, 1, 2, 300.0);
    assert_store_number_tol(&cell_store, 0, 2, 2, bear_pt, 0.01);
    assert_store_number_tol(&cell_store, 0, 3, 2, bear_tax, 0.01);
    assert_store_number_tol(&cell_store, 0, 4, 2, bear_ni, 0.01);
...
fn test_debug_broken_formulas_incremental() {
    ...
    set(&mut core, &mut cell_store, 0, 0, 0, "100");
    assert_store_number(&cell_store, 0, 0, 0, 100.0);

    // Step 2: Set A2 = "=A1*" (parse error — incomplete expression)
    set(&mut core, &mut cell_store, 0, 1, 0, "=A1*");
    // Parse error produces an error value
    assert_store_is_any_error(&cell_store, 0, 1, 0);

    // Step 3: Set A3 = "=A2+10" — depends on broken A2
    set(&mut core, &mut cell_store, 0, 2, 0, "=A2+10");
    // A2 is an error, so A3 propagates error
    assert_store_is_any_error(&cell_store, 0, 2, 0);

    // Step 4: Set A4 = "=NOSUCHFUNC(A1)" — unknown function
    set(&mut core, &mut cell_store, 0, 3, 0, "=NOSUCHFUNC(A1)");
    assert_store_is_any_error(&cell_store, 0, 3, 0);

    // Step 5: Set A5 = "=1/0" — #DIV/0!
    set(&mut core, &mut cell_store, 0, 4, 0, "=1/0");
    assert_store_error(&cell_store, 0, 4, 0, CellError::Div0);

    // Step 6: Fix A2 — correct formula "=A1*2" = 200
    set(&mut core, &mut cell_store, 0, 1, 0, "=A1*2");
    assert_store_number(&cell_store, 0, 1, 0, 200.0);

    // Step 7: A3 should now cascade-fix = A2+10 = 210
    assert_store_number(&cell_store, 0, 2, 0, 210.0);

    // Step 8: Fix A4 — correct formula "=SUM(A1)" = 100
    set(&mut core, &mut cell_store, 0, 3, 0, "=SUM(A1)");
    assert_store_number(&cell_store, 0, 3, 0, 100.0);

    // Step 9: Fix A5 — correct formula "=A1/2" = 50
    set(&mut core, &mut cell_store, 0, 4, 0, "=A1/2");
    assert_store_number(&cell_store, 0, 4, 0, 50.0);

    // Step 10: Add A6 = "=A2+A3+A4+A5" = 200+210+100+50 = 560
    set(&mut core, &mut cell_store, 0, 5, 0, "=A2+A3+A4+A5");
    assert_store_number(&cell_store, 0, 5, 0, 560.0);
...
fn test_full_60_step_agent_workflow() {
    ...
    set(&mut core, &mut cell_store, 0, 0, 1, "0.4");
    assert_store_number(&cell_store, 0, 0, 1, 0.4);

    // Step 3: Tax rate B2 = 0.3
    set(&mut core, &mut cell_store, 0, 1, 1, "0.3");
    assert_store_number(&cell_store, 0, 1, 1, 0.3);

    // Step 4: COGS A2 = "=A1*B1" = 1000*0.4 = 400
    set(&mut core, &mut cell_store, 0, 1, 0, "=A1*B1");
    assert_store_number(&cell_store, 0, 1, 0, 400.0);
    ...
    // Step 6: OpEx A4 = 150
    set(&mut core, &mut cell_store, 0, 3, 0, "150");
    assert_store_number(&cell_store, 0, 3, 0, 150.0);

    // Step 7: Depreciation A5 = 50
    set(&mut core, &mut cell_store, 0, 4, 0, "50");
    assert_store_number(&cell_store, 0, 4, 0, 50.0);

    // Step 8: EBIT A6 = "=A3-A4-A5" = 600-150-50 = 400
    set(&mut core, &mut cell_store, 0, 5, 0, "=A3-A4-A5");
    assert_store_number(&cell_store, 0, 5, 0, 400.0);

    // Step 9: Interest A7 = 30
    set(&mut core, &mut cell_store, 0, 6, 0, "30");
    assert_store_number(&cell_store, 0, 6, 0, 30.0);

    // Step 10: PreTax A8 = "=A6-A7" = 400-30 = 370 (non-circular first)
    set(&mut core, &mut cell_store, 0, 7, 0, "=A6-A7");
    assert_store_number(&cell_store, 0, 7, 0, 370.0);

    // Step 11: Tax A9 = "=A8*B2" = 370*0.3 = 111 (non-circular)
    set(&mut core, &mut cell_store, 0, 8, 0, "=A8*B2");
    assert_store_number(&cell_store, 0, 8, 0, 111.0);

    // Step 12: NetIncome A10 = "=A8-A9" = 370-111 = 259
    set(&mut core, &mut cell_store, 0, 9, 0, "=A8-A9");
    assert_store_number(&cell_store, 0, 9, 0, 259.0);
    ...
    assert_store_number_tol(&cell_store, 0, 7, 0, expected_pt, 0.01);
    assert_store_number_tol(&cell_store, 0, 8, 0, expected_tax, 0.01);
    assert_store_number_tol(&cell_store, 0, 9, 0, expected_ni, 0.01);
    ...
    for step in 0u32..10 {
        ...
        assert_store_number(&cell_store, 0, 0, 0, revenue);
        assert_store_number(&cell_store, 0, 1, 0, cogs);
        assert_store_number(&cell_store, 0, 2, 0, gross);
        assert_store_number(&cell_store, 0, 5, 0, ebit);
        assert_store_number_tol(&cell_store, 0, 7, 0, pt, 0.01);
        assert_store_number_tol(&cell_store, 0, 8, 0, tax, 0.01);
        assert_store_number_tol(&cell_store, 0, 9, 0, ni, 0.01);
    }

    // After loop, Revenue = 5500
    assert_store_number(&cell_store, 0, 0, 0, 5500.0);

    // === Pass 3: Add more cells (Steps 26-35) ===

    // Step 26: Change depreciation to formula: A5 = "=A1*0.05" = 5500*0.05 = 275
    set(&mut core, &mut cell_store, 0, 4, 0, "=A1*0.05");
    assert_store_number(&cell_store, 0, 4, 0, 275.0);

    // Step 27: EBIT recalculates = Gross - OpEx - Depr = 3300-150-275 = 2875
    assert_store_number(&cell_store, 0, 5, 0, 2875.0);

    // Step 28: G = EBIT - Interest = 2875-30 = 2845
    // PreTax = 2845/1.3, Tax = 2845*0.3/1.3, NI = 2845*0.7/1.3
    let g28 = 2845.0;
    assert_store_number_tol(&cell_store, 0, 7, 0, g28 / 1.3, 0.01);
    assert_store_number_tol(&cell_store, 0, 8, 0, g28 * 0.3 / 1.3, 0.01);
    assert_store_number_tol(&cell_store, 0, 9, 0, g28 * 0.7 / 1.3, 0.01);

    // Step 29: Change Interest to formula: A7 = "=2000*0.04" = 80
    set(&mut core, &mut cell_store, 0, 6, 0, "=2000*0.04");
    assert_store_number(&cell_store, 0, 6, 0, 80.0);

    // Step 30: G = 2875-80 = 2795
    let g30 = 2795.0;
    assert_store_number_tol(&cell_store, 0, 7, 0, g30 / 1.3, 0.01);
    assert_store_number_tol(&cell_store, 0, 8, 0, g30 * 0.3 / 1.3, 0.01);

    // Step 31: Add RetainedEarnings A11 = "=A10" (same as NI)
    set(&mut core, &mut cell_store, 0, 10, 0, "=A10");
    let ni30 = g30 * 0.7 / 1.3;
    assert_store_number_tol(&cell_store, 0, 10, 0, ni30, 0.01);

    // Step 32: Add Dividend A12 = "=A11*0.4" (40% payout)
    set(&mut core, &mut cell_store, 0, 11, 0, "=A11*0.4");
    assert_store_number_tol(&cell_store, 0, 11, 0, ni30 * 0.4, 0.01);

    // Step 33: Add RetainedAfterDiv A13 = "=A11-A12" = NI*0.6
    set(&mut core, &mut cell_store, 0, 12, 0, "=A11-A12");
    assert_store_number_tol(&cell_store, 0, 12, 0, ni30 * 0.6, 0.01);
    ...
    assert_store_number(&cell_store, 0, 5, 0, 400.0);
    assert_store_number(&cell_store, 0, 6, 0, 80.0);
    assert_store_number_tol(&cell_store, 0, 7, 0, g35 / 1.3, 0.01);
    assert_store_number_tol(&cell_store, 0, 8, 0, g35 * 0.3 / 1.3, 0.01);
    let ni35 = g35 * 0.7 / 1.3;
    assert_store_number_tol(&cell_store, 0, 9, 0, ni35, 0.01);
    assert_store_number_tol(&cell_store, 0, 10, 0, ni35, 0.01);
    assert_store_number_tol(&cell_store, 0, 11, 0, ni35 * 0.4, 0.01);
    assert_store_number_tol(&cell_store, 0, 12, 0, ni35 * 0.6, 0.01);

    // === Pass 4: Clear and rebuild (Steps 36-45) ===

    // Step 36: Clear Depreciation — set to constant 0
    set(&mut core, &mut cell_store, 0, 4, 0, "0");
    assert_store_number(&cell_store, 0, 4, 0, 0.0);

    // Step 37: EBIT = 600-150-0 = 450
    assert_store_number(&cell_store, 0, 5, 0, 450.0);

    // Step 38: G = 450-80 = 370, verify cascade
    let g38 = 370.0;
    assert_store_number_tol(&cell_store, 0, 7, 0, g38 / 1.3, 0.01);

    // Step 39: Clear Interest — set to 0
    set(&mut core, &mut cell_store, 0, 6, 0, "0");
    assert_store_number(&cell_store, 0, 6, 0, 0.0);

    // Step 40: G = EBIT - 0 = 450
    let g40 = 450.0;
    assert_store_number_tol(&cell_store, 0, 7, 0, g40 / 1.3, 0.01);
    assert_store_number_tol(&cell_store, 0, 8, 0, g40 * 0.3 / 1.3, 0.01);

    // Step 41: Re-add Depreciation = "=A1*0.1" = 100
    set(&mut core, &mut cell_store, 0, 4, 0, "=A1*0.1");
    assert_store_number(&cell_store, 0, 4, 0, 100.0);

    // Step 42: EBIT = 600-150-100 = 350
    assert_store_number(&cell_store, 0, 5, 0, 350.0);

    // Step 43: G = 350-0 = 350
    let g43 = 350.0;
    assert_store_number_tol(&cell_store, 0, 7, 0, g43 / 1.3, 0.01);

    // Step 44: Re-add Interest = "=3000*0.05" = 150
    set(&mut core, &mut cell_store, 0, 6, 0, "=3000*0.05");
    assert_store_number(&cell_store, 0, 6, 0, 150.0);

    // Step 45: G = 350-150 = 200
    let g45 = 200.0;
    assert_store_number_tol(&cell_store, 0, 7, 0, g45 / 1.3, 0.01);
    assert_store_number_tol(&cell_store, 0, 8, 0, g45 * 0.3 / 1.3, 0.01);
    let ni45 = g45 * 0.7 / 1.3;
    assert_store_number_tol(&cell_store, 0, 9, 0, ni45, 0.01);

    // === Pass 5: Final adjustments (Steps 46-60) ===

    // Step 46: Change margin to 0.3 (B1)
    set(&mut core, &mut cell_store, 0, 0, 1, "0.3");
    assert_store_number(&cell_store, 0, 0, 1, 0.3);

    // Step 47: COGS = 1000*0.3 = 300
    assert_store_number(&cell_store, 0, 1, 0, 300.0);

    // Step 48: Gross = 1000-300 = 700
    assert_store_number(&cell_store, 0, 2, 0, 700.0);
    ...
    let g50 = 300.0;
    assert_store_number_tol(&cell_store, 0, 7, 0, g50 / 1.3, 0.01);

    // Step 51: Change tax rate to 0.25 (B2)
    set(&mut core, &mut cell_store, 0, 1, 1, "0.25");
    assert_store_number(&cell_store, 0, 1, 1, 0.25);

    // Step 52: G stays 300 (tax rate doesn't affect EBIT)
    // PreTax = 300/1.25 = 240, Tax = 300*0.25/1.25 = 60, NI = 240-60 = 180
    assert_store_number_tol(&cell_store, 0, 7, 0, 300.0 / 1.25, 0.01);
    assert_store_number_tol(&cell_store, 0, 8, 0, 300.0 * 0.25 / 1.25, 0.01);
    let ni52 = 300.0 / 1.25 - 300.0 * 0.25 / 1.25;
    assert_store_number_tol(&cell_store, 0, 9, 0, ni52, 0.01);

    // Step 53: RetainedEarnings should cascade
    assert_store_number_tol(&cell_store, 0, 10, 0, ni52, 0.01);

    // Step 54: Dividend = NI * 0.4
    assert_store_number_tol(&cell_store, 0, 11, 0, ni52 * 0.4, 0.01);

    // Step 55: RetainedAfterDiv = NI * 0.6
    assert_store_number_tol(&cell_store, 0, 12, 0, ni52 * 0.6, 0.01);

    // Step 56: Revenue bump to 3000
    set(&mut core, &mut cell_store, 0, 0, 0, "3000");
    assert_store_number(&cell_store, 0, 0, 0, 3000.0);

    // Step 57: COGS = 3000*0.3 = 900, Gross = 2100
    assert_store_number(&cell_store, 0, 1, 0, 900.0);
    assert_store_number(&cell_store, 0, 2, 0, 2100.0);

    // Step 58: Depr = 3000*0.1 = 300, EBIT = 2100-150-300 = 1650
    assert_store_number(&cell_store, 0, 4, 0, 300.0);
    assert_store_number(&cell_store, 0, 5, 0, 1650.0);

    // Step 59: G = 1650-150 = 1500, rate=0.25
    // PreTax = 1500/1.25 = 1200, Tax = 1500*0.25/1.25 = 300, NI = 900
    let g59 = 1500.0;
    assert_store_number_tol(&cell_store, 0, 7, 0, g59 / 1.25, 0.01);
    assert_store_number_tol(&cell_store, 0, 8, 0, g59 * 0.25 / 1.25, 0.01);
    let ni59 = g59 / 1.25 - g59 * 0.25 / 1.25; // 1200 - 300 = 900
    assert_store_number_tol(&cell_store, 0, 9, 0, ni59, 0.01);

    // Step 60: Final state verification — every cell
    assert_store_number(&cell_store, 0, 0, 0, 3000.0); // A1  Revenue
    assert_store_number(&cell_store, 0, 0, 1, 0.3); // B1  Margin
    assert_store_number(&cell_store, 0, 1, 1, 0.25); // B2  TaxRate
    assert_store_number(&cell_store, 0, 1, 0, 900.0); // A2  COGS
    assert_store_number(&cell_store, 0, 2, 0, 2100.0); // A3  Gross
    assert_store_number(&cell_store, 0, 3, 0, 150.0); // A4  OpEx
    assert_store_number(&cell_store, 0, 4, 0, 300.0); // A5  Depreciation
    assert_store_number(&cell_store, 0, 5, 0, 1650.0); // A6  EBIT
    assert_store_number(&cell_store, 0, 6, 0, 150.0); // A7  Interest
    assert_store_number_tol(&cell_store, 0, 7, 0, 1200.0, 0.01); // A8  PreTax
    assert_store_number_tol(&cell_store, 0, 8, 0, 300.0, 0.01); // A9  Tax
    assert_store_number_tol(&cell_store, 0, 9, 0, 900.0, 0.01); // A10 NetIncome
    assert_store_number_tol(&cell_store, 0, 10, 0, 900.0, 0.01); // A11 RetainedEarnings
    assert_store_number_tol(&cell_store, 0, 11, 0, 360.0, 0.01); // A12 Dividend (900*0.4)
    assert_store_number_tol(&cell_store, 0, 12, 0, 540.0, 0.01); // A13 RetainedAfterDiv (900*0.6)
}
