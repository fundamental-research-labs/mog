...
//! Each test imports it through `ComputeEngine`, performs the edit through
...
fn imported_engine() -> (ComputeEngine, SheetId, String) {
    imported_engine_from_xlsx(chart_fixture_xlsx())
}

fn imported_title_formula_engine() -> (ComputeEngine, SheetId, String) {
    imported_engine_from_xlsx(title_formula_chart_fixture_xlsx())
}

fn imported_engine_from_xlsx(xlsx: Vec<u8>) -> (ComputeEngine, SheetId, String) {
    let (engine, _) = ComputeEngine::from_xlsx_bytes(&xlsx)
