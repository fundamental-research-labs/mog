...
//! the parser must keep it typed, Yrs must persist it without registering a
...
#[test]
fn empty_formula_metadata_survives_parse_yrs_export_without_formula_registration() {
    ...
    let (engine, _) = YrsComputeEngine::from_xlsx_bytes(&source).expect("hydrate source fixture");
