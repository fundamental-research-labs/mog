...
| Headless agent routes are absent, disabled, or constrained in a given enterprise deployment. | Not claimed | The public runtime service contracts include optional agent policy fields, including an agent URL and bypass token. Enterprise distributions must explicitly disable or constrain any agent routes before claiming no AI/agent egress. |
