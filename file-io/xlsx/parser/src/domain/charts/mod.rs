...
fn is_self_closing_tag(xml: &[u8], pos: usize) -> bool {
...
fn find_pos_after_last_ser(xml: &[u8]) -> usize {
    let mut pos = 0;
    let mut last_end = 0;
...
fn parse_ax_ids(xml: &[u8]) -> Vec<u32> {
    let mut ids = Vec::new();
    let mut pos = 0;
...
fn parse_chart_type_ext_lst(xml: &[u8]) -> Vec<ooxml_types::charts::ExtensionEntry> {
...
fn is_ext_lst_tag(bytes: &[u8]) -> bool {
