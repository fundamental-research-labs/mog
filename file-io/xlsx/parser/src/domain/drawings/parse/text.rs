...
pub fn parse_text_body(xml: &[u8]) -> Option<TextBody> {
    ...
    for child in direct_child_elements(xml) {
        ...
        match child.local_name {
            b"bodyPr" => body.body_props = parse_body_props(child_xml),
            b"lstStyle" => body.list_style = parse_list_style(child_xml),
            b"p" => {
                if let Some(para) = parse_paragraph(child_xml) {
...
fn parse_direct_fill_choice(xml: &[u8]) -> Option<Fill> {
