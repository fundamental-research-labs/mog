...
///
/// This is the handoff shape sibling parsers should consume instead of deriving
/// owner paths from `sheet_num` or `xl/worksheets/sheetN.xml`.
    ...
    for (workbook_order, sheet) in sheet_infos.iter().enumerate() {
        ...
        if relationship_id_count > 1 {
            diagnostics.push(package_diag(
                ...
                format!("workbook relationship {} appears {relationship_id_count} times", sheet.r_id),
            ));
        }
        ...
        if let Some(rel) = relationship {
                ...
                .is_some_and(|mode| mode.eq_ignore_ascii_case("External"))
            {
                diagnostics.push(package_diag(
                    ...
                    format!("workbook sheet relationship {} targets an external resource", rel.id),
                ));
            } else {
...
fn classify_sheet_kind(
    ...
    match (relationship_type, content_type) {
        ...
        | (Some(REL_MACRO_SHEET), Some(_)) => {
            WorkbookSheetKind::Invalid
        }
