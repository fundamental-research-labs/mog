...
use crate::output::results::{
    ...
    HyperlinkOutput, ParseStats, ParseTimings, ProtectionOutput, RawVmlDrawing,
    SmartArtPartsOutput, SparklineSummary, StylesOutput,
};
...
fn extract_root_start_tag(xml: &[u8]) -> Option<&[u8]> {
    let mut pos = 0;
...
fn capture_namespaces_from_xml(xml: &[u8]) -> NamespaceMap {
...
fn capture_ext_lst_raw(xml: &[u8]) -> Option<Vec<u8>> {
...
fn extract_worksheet_ext_lst_xml(post_sd: &[u8]) -> Option<String> {
    use crate::infra::scanner::find_tag_simd;
...
fn extract_raw_element_xml(xml: &[u8], tag: &[u8]) -> Option<String> {
...
fn extract_worksheet_controls_xml(xml: &[u8]) -> Option<String> {
...
fn extract_explicit_blank_cells(xml: &[u8]) -> Vec<(u32, u32)> {
...
fn find_enclosing_alternate_content_start(xml: &[u8], before: usize) -> Option<usize> {
    let mut pos = 0;
    let mut last = None;
...
fn find_matching_alternate_content_end(xml: &[u8], start: usize) -> Option<usize> {
    let mut pos = start;
...
fn is_known_workbook_child(tag_name: &[u8]) -> bool {
...
fn capture_workbook_preserved_elements(xml: &[u8]) -> PreservedElements {
...
fn is_known_child(tag_name: &[u8], known_list: &[&[u8]]) -> bool {
    let local = if let Some(colon_pos) = memchr::memchr(b':', tag_name) {
        &tag_name[colon_pos + 1..]
    } else {
        tag_name
...
fn parse_dimension_ref(pre_sd: &[u8]) -> Option<String> {
...
fn capture_preserved_in_region(
    xml: &[u8],
...
fn capture_preserved_in_region_with_anchor(
    xml: &[u8],
    region_start_in_xml: usize,
    region: &[u8],
    parent_path: &str,
...
fn find_post_sheet_data_region(xml: &[u8]) -> &[u8] {
...
fn parse_external_reference_rids(workbook_xml: &[u8]) -> Vec<String> {
...
fn capture_sheet_preserved_elements(
    worksheet_xml: &[u8],
    pre_sd: &[u8],
    post_sd: &[u8],
) -> PreservedElements {
