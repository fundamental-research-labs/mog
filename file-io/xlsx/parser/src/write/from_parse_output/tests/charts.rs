...
#[test]
fn authoritative_chart_replay_preserves_chart_owned_image_part() {
    ...
    assert_eq!(
        archive.read_file("xl/media/image23.jpeg").unwrap(),
        b"\xff\xd8\xff\x00chart-fill"
    );
    let chart_rels = String::from_utf8(
        archive
    ...
    assert!(chart_rels.contains(r#"Target="../media/image23.jpeg""#));
    validate_archive_package_integrity(&archive).expect("exported package should be valid");
}
