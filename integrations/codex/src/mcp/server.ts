...
type BrowserRpcPayload =
  | Omit<Extract<BrowserRpcRequest, { type: 'cell_read' }>, 'requestId'>
  | Omit<Extract<BrowserRpcRequest, { type: 'cell_write' }>, 'requestId'>
  | Omit<Extract<BrowserRpcRequest, { type: 'selection_set' }>, 'requestId'>
  | Omit<Extract<BrowserRpcRequest, { type: 'export_xlsx' }>, 'requestId'>
  | Omit<Extract<BrowserRpcRequest, { type: 'session_close' }>, 'requestId'>;
...
const pluginVersion = typeof MOG_PLUGIN_VERSION === 'string' ? MOG_PLUGIN_VERSION : '0.10.8-dev.197ea3a6f';
const wasmPackageBaseUrl =
  typeof MOG_WASM_PACKAGE_BASE_URL === 'string'
    ? MOG_WASM_PACKAGE_BASE_URL
    : `https://cdn.jsdelivr.net/npm/@mog-sdk/wasm@${pluginVersion}/`;
...
function textResponse(response: ServerResponse, statusCode: number, body: string): void {
  response.writeHead(statusCode, {
...
function updateSessionStatus(
  ...
  patch: Partial<Omit<BrowserStatus, 'updatedAt'>>,
): void {
  session.status = {
    ...session.status,
    ...patch,
...
async function handleBrowserRequest(
  ...
  if (request.method === 'GET' && sessionMatch) {
    const session = getSession(response, sessionMatch[1], url.searchParams.get('token'));
    if (!session) return;
    updateSessionStatus(session, { connected: true, smokeStatus: 'loading' });
  ...
  if (request.method === 'POST' && operation === 'status') {
    ...
    updateSessionStatus(session, body as Partial<BrowserStatus>);
    jsonResponse(response, 200, { ok: true });
    return;
  }

  if (request.method === 'POST' && operation === 'rpc-result') {
    const body = (await readJsonBody(request)) as BrowserRpcResult;
...
function resolveExplicitPath(input: string, label: string): string {
...
async function sourceFromArgs(args: Record<string, unknown>): Promise<BrowserSource> {
  const pathInput = args.xlsxPath ?? args.filePath ?? args.path;
  if (pathInput === undefined || pathInput === null || pathInput === '') {
    return { kind: 'blank' };
  }
...
async function createBrowserSession(args: Record<string, unknown>): Promise<ToolResult> {
...
function sessionStatus(args: Record<string, unknown>): ToolResult {
...
function rpcToBrowser(
  session: BrowserSession,
  request: BrowserRpcPayload,
  timeoutMs = 60000,
): Promise<JsonValue> {
  ...
  const payload: BrowserRpcRequest = { requestId, ...request } as BrowserRpcRequest;
...
function requireSession(args: Record<string, unknown>): BrowserSession {
  const sessionId = String(args.sessionId ?? '');
  const session = sessions.get(sessionId);
  if (!session) {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<ToolResult> {
  switch (name) {
    ...
    case 'mog_export_xlsx': {
      ...
      const result = await rpcToBrowser(session, { type: 'export_xlsx' }, 120000);
      const exportPayload = result as { readonly bytesBase64?: JsonValue };
      if (
        !result ||
        typeof result !== 'object' ||
        Array.isArray(result) ||
        typeof exportPayload.bytesBase64 !== 'string'
      ) {
...
function textToolResult(value: unknown): { content: [{ type: 'text'; text: string }] } {
  return {
    content: [
      {
        type: 'text',
...
function mcpError(id: unknown, code: number, message: string): void {
  writeMcp({ jsonrpc: '2.0', id, error: { code, message } });
}

async function handleMcpRequest(message: any): Promise<void> {
      ...
      case 'initialize':
        writeMcp({
          ...
          result: {
            ...
            instructions:
              'Use Mog tools to start a localhost browser session, open the returned URL in the Codex in-app browser, wait for readiness, then operate on that visible workbook.',
          },
        });
        return;
