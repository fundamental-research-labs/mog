...
export function registerSnapshotRootMaterializationScenarios(): void {
  it(
    'materializes normal commit snapshot roots through the configured byte-sync port',
    async () => {
  ...
  it(
    'keeps provider-failed diagnostics when materialized snapshot-root capture fails',
    async () => {
      const provider = createInMemoryVersionStoreProvider({ documentScope: DOCUMENT_SCOPE });
      const initialized = await provider.initializeGraph(await initializeInput('graph-1', 'root'));
      expectInitializeSuccess(initialized);
