import type { DrawingHandle, WorksheetDrawingCollection } from '@mog-sdk/contracts/api';
...
export class WorksheetDrawingCollectionImpl implements WorksheetDrawingCollection {
  ...
  ): Promise<DrawingHandle> {
    const drawingId = await this.objectsImpl.createDrawing(position, options);
    return new DrawingHandleImpl(drawingId, this.objectsImpl, this.boundsReader);
  }
}
