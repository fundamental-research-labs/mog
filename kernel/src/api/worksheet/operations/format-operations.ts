...
type FormatOperationMutationOptions = MutationAdmissionOptions & {
...
export async function setFormat(
  ...
  try {
    ...
    if (options) {
      ...
      await ctx.computeBridge.setFormatForRanges(sheetId, [[row, col, row, col]], normalizedFormat);
    }
    return {
      success: true,
      data: undefined,
...
export async function setFormatForRanges(
  ...
  return wrapOp('setFormatForRanges', async () => {
    ...
    for (const range of ranges) {
      if (range.isFullColumn) {
        for (let col = range.startCol; col <= range.endCol; col++) {
          ...
          promises.push(
            admissionOptions
              ? ctx.computeBridge.setColFormat(sheetId, col, normalizedFormat, admissionOptions)
              : ctx.computeBridge.setColFormat(sheetId, col, normalizedFormat),
          );
        }
        ...
        for (let row = range.startRow; row <= range.endRow; row++) {
          ...
          promises.push(
            admissionOptions
              ? ctx.computeBridge.setRowFormat(sheetId, row, normalizedFormat, admissionOptions)
              : ctx.computeBridge.setRowFormat(sheetId, row, normalizedFormat),
          );
        }
      } else {
    ...
    if (boundedRanges.length > 0) {
      ...
      promises.push(
        admissionOptions
          ? ctx.computeBridge.setFormatForRanges(
              sheetId,
              boundedRanges,
              normalizedFormat,
              admissionOptions,
            )
          : ctx.computeBridge.setFormatForRanges(sheetId, boundedRanges, normalizedFormat),
      );
    }
...
export async function setRowFormat(
  ...
  return wrapOp('setRowFormat', async () => {
    ...
    if (options) {
      await ctx.computeBridge.setRowFormat(sheetId, row, normalizedFormat, options);
    } else {
      await ctx.computeBridge.setRowFormat(sheetId, row, normalizedFormat);
    }
  });
}
...
export async function setColFormat(
  ...
  return wrapOp('setColFormat', async () => {
    ...
    if (options) {
      await ctx.computeBridge.setColFormat(sheetId, col, normalizedFormat, options);
    } else {
      await ctx.computeBridge.setColFormat(sheetId, col, normalizedFormat);
    }
  });
}
...
export async function clearFormat(
  ...
  try {
    await assertFormatOperationsAllowed(ctx, sheetId, ['formatCells']);
    if (options) {
      await ctx.computeBridge.clearFormatForRanges(sheetId, [[row, col, row, col]], options);
    } else {
...
export async function applyFormatToRange(
  ...
  if (!sourceRange) {
    await setFormatForRanges(ctx, sheetId, [targetRange], normalizedSourceFormat, options);
    return;
  }
  ...
  for (const [key, ranges] of formatToRanges) {
    ...
    ipcPromises.push(
      admissionOptions
        ? ctx.computeBridge.setFormatForRanges(sheetId, ranges, format, admissionOptions)
        : ctx.computeBridge.setFormatForRanges(sheetId, ranges, format),
    );
  }
...
export async function setRowProperties(
  ...
  return wrapOp('setRowProperties', async () => {
    ...
    await assertFormatOperationsAllowed(ctx, sheetId, ['formatRows']);
    const entries: Array<[number, CellFormat]> = Array.from(normalizedUpdates.entries());
    if (options) {
...
export async function setCellProperties(
  ...
  return wrapOp('setCellProperties', async () => {
    ...
    await assertFormatOperationsAllowed(ctx, sheetId, ['formatCells']);
    const tuples: Array<[number, number, CellFormat]> = normalizedUpdates.map((u) => [
      u.row,
      u.col,
      u.format,
    ]);
...
function createGroupedFormatOperationOptions(
  ctx: DocumentContext,
  sheetId: SheetId,
  operationIdPrefix: string,
  options?: MutationAdmissionOptions,
): () => MutationAdmissionOptions | undefined {
  if (!hasOperationContext(options)) {
    return () => options;
  }

  let nextOptions = ensureFormatOperationGroup(options);
  ...
  return () => {
    ...
    nextOptions = {
      ...
      operationContext: createVersionOperationContext(ctx, {
        operationIdPrefix,
        sheetIds: [sheetId],
        domainIds: options.operationContext.domainIds,
        groupId,
      }),
    };
...
function hasOperationContext(
  options?: MutationAdmissionOptions,
): options is FormatOperationMutationOptions {
  return options?.operationContext != null;
}

function ensureFormatOperationGroup(
  options: FormatOperationMutationOptions,
): FormatOperationMutationOptions {
