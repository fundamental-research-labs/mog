...
export { type EditorState } from '@mog-sdk/contracts/actors/editor';
...
export const editorSelectors = {
  ...
  sheetId: (state: EditorState): EditorState['context']['sheetId'] => state.context.sheetId,

  /** Get the merged region bounds if editing a merged cell */
  mergeBounds: (state: EditorState): EditorState['context']['mergeBounds'] =>
    state.context.mergeBounds,
  ...
  errorMessage: (state: EditorState): EditorState['context']['errorMessage'] =>
    state.context.errorMessage,

  /** Get the commit direction */
  commitDirection: (state: EditorState): EditorState['context']['commitDirection'] =>
    state.context.commitDirection,
  ...
  editorType: (state: EditorState): EditorState['context']['editorType'] =>
    state.context.editorType,

  /** Get the cell schema */
  cellSchema: (state: EditorState): EditorState['context']['cellSchema'] =>
    state.context.cellSchema,

  /** Get the enum items for dropdown */
  enumItems: (state: EditorState): EditorState['context']['enumItems'] => state.context.enumItems,
  ...
  richTextSegments: (state: EditorState): EditorState['context']['richTextSegments'] =>
    state.context.richTextSegments,
  ...
  currentFormat: (state: EditorState): EditorState['context']['currentFormat'] =>
    state.context.currentFormat,

  /** Get formula context for autocomplete (function name, argument position, etc.) */
  formulaContext: (state: EditorState): EditorState['context']['formulaContext'] =>
    state.context.formulaContext,
