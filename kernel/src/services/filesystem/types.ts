 ...
 *
 * This module defines the core filesystem interface that can be implemented
...
/**
 * Public interface for the kernel filesystem service.
 *
 * Manages per-app sandboxed filesystems and provides access
 * to the underlying backend for system-level operations.
 *
 * Note: This is the *service wrapper* interface — distinct from
 * `IFileSystem` which is the low-level backend interface.
 */
export interface IFilesystemService extends IDisposable {
  ...
  initializeApp(appId: AppId): Promise<void>;

  /** Remove all files for an app (for uninstallation). */
  removeApp(appId: AppId): Promise<void>;
