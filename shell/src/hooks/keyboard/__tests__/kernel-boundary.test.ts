...
const SOURCE_ROOT = resolve(SHELL_ROOT, 'src');
const KEYBOARD_HOOK_ROOT = resolve(SHELL_ROOT, 'src/hooks/keyboard');
...
function toShellRelative(filePath: string): string {
  return relative(SHELL_ROOT, filePath).replaceAll('\\', '/');
}

describe('keyboard hook kernel boundary', () => {
  it('keeps keyboard hooks off the kernel internal barrel', () => {
    const violations = collectSourceFiles(KEYBOARD_HOOK_ROOT)
  ...
  it('does not import keyboard symbols from the kernel internal barrel anywhere in shell source', () => {
    const violations = collectSourceFiles(SOURCE_ROOT)
      .map((file) => {
        const imported = getKernelInternalKeyboardSymbols(readFileSync(file, 'utf8'));
        return { file: toShellRelative(file), imported };
      })
